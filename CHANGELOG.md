...
- `release.yml` signed release reliability smoke now passes the secret-pair
  public key as `MAEKON_UPDATE_PUBLIC_KEY` env to `release-reliability-smoke.sh`.
  Previously the env was unset and `install.sh` fell back to its hardcoded
  default, which did not match the secret pair used to sign the artifacts.
  Surfaced on v0.0.1-rc.2 release.yml run 25372738839 (`Create GitHub Release`
  job, `Run signed release reliability smoke` step).
- `install.sh` hardcoded fallback `UPDATE_SIGNATURE_PUBLIC_KEY` updated from
  the stale rebrand-era value to the current public key matching the
  `UPDATE_SIGNING_PRIVATE_KEY_B64` secret. Ensures end-user
  `curl install.sh | bash` automatic signature verification passes against
  artifacts published by this release pipeline.

## [0.0.1-rc.2] - 2026-05-05

### Fixed

- `Verify MSI version` step in `release.yml` no longer false-positives on the
  `0.0.1` line. The previous `if ($msiName -match "0\.0\.1" ...)` guard was
  hardcoded against the old `0.4.41` line and never tripped there, but on the
  new `0.0.1-rc.1` MSI filename `maekon-app-0.0.1.1-x86_64.msi` (Tauri
  normalizes `-rc.1` → `.1` per the WiX 4-part numeric version constraint)
  it tripped on every Windows build. The check now derives the expected MSI
  pattern from the tag (`-rc.N` → `.N`) and verifies via `[regex]::Escape`
  with no hardcoded version literal.

## [0.0.1-rc.1] - 2026-05-05
...
- Migrate `rustls-pemfile` to `rustls-pki-types::PemObject` to resolve
  RUSTSEC-2025-0134 (`rustls-pemfile` archived August 2025) across the
  maekon-web and maekon-network TLS load paths
  ([#1286](https://github.com/pseudotop/maekon-client/pull/1286)).
- Tighten `cargo-deny` `unmaintained` scope to workspace-only so transitive
  crate maintenance status no longer fails the supply-chain gate
  ([#1289](https://github.com/pseudotop/maekon-client/pull/1289)).

### Changed

- Document the public release alert gate in the release runbook
  ([#1192](https://github.com/pseudotop/maekon-client/pull/1192)).
...
- Align Release Smoke build flags with release.yml ([#518](https://github.com/pseudotop/oneshim-client/pull/518))
  ...
  `maekon-embedding` -> `fastembed` -> `ort-sys`, and `ort-sys`
...
- **🔥 D5 PII filter audit — CRITICAL clipboard fix + 10 gap closures** (internal design and audit-matrix notes).
  ...
  - **Hexagonal arch compliance**: adapter crates (`maekon-monitor`, `maekon-audio`, `maekon-network`, `maekon-automation`, `maekon-analysis`) use injected `Arc<dyn PiiSanitizer>` via `with_pii_sanitizer` builder — no direct `maekon-vision` dep.
...
- **D5 iter-16 — `SanitizedDisplay<T>` wrapper for PII-safe error logging** (audit matrix Paths 10 + 22). Addresses the deferred iter-16 design requirement with an addition-only wrapper that sidesteps the `thiserror` macro conflict.
  - **New in `maekon-core`**: [`sanitized_display.rs`](crates/maekon-core/src/sanitized_display.rs) exports `SanitizedDisplay<T: Display>` + `sanitized(&value, sanitizer, level)` helper. Wraps any `Display`-implementing value (including `CoreError`, `dyn Error`, `String`) so its formatted output is routed through a `PiiSanitizer` port before reaching the `Formatter`.
  ...
  - **Scope**: addition-only. Existing `tracing::*` call-sites in `maekon-network`, scheduler loops, and elsewhere remain unchanged; site-by-site migration happens in follow-up PRs on a per-adapter basis.
...
- **D5 iter-16 — SanitizedDisplay migration rollout across LLM-error tracing sites** (audit matrix Path 22 follow-up). All 3 high-PII-risk `CoreError::Display` tracing sites migrated; low-risk infrastructure sites documented-skipped.
  - **Migrated** (3 adapters, 3 PRs):
    - `maekon-analysis::LlmWorkTypeRefiner` — LLM work-type classification error logging (PR #458).
...
- **ADR-019** Error Code Infrastructure + C5 AWS Bedrock intentional non-support. See [ADR-019](docs/architecture/ADR-019-error-code-infrastructure.md) (with Korean companion).
  ...
  - **Single-source wire contract**: 18 typed code enums under `crates/maekon-core/src/error_codes/` generated via a `define_code_enum!` macro. Wire-format contract locked via `crates/maekon-core/tests/wire_contract_snapshot.rs`.
  - **AWS Bedrock skip (C5)**: Bedrock deleted from `specs/providers/provider-surface-catalog.json`. 8 match arms in `maekon-network` + 2 defense-in-depth guards (`analysis_client`, `ai_model_catalog_web_service`) return `ConfigCode::UnsupportedProviderBedrock`. Closes the silent no-auth fallthrough security bug in OCR `apply_auth_headers`, plus 3 additional Bedrock defects found during drift audit (2 silent-fallthrough sibling paths + 1 wildcard-mislabel in `http_api_session::build_request_body`).
...
- **A-4** Release metadata enrichment: `cliff.toml` body adds "Release Date" + "Since v{prev}" lines; `.github/workflows/release.yml` prepends "## MAEKON Client {TAG} — Released {Date}"; frontend `UpdatePanel` surfaces `published_at` for pending updates and renders the new `RolledBack` phase with from/to versions + dates + reason.
...
- Create sandbox-worker stub for release smoke Linux job ([#434](https://github.com/pseudotop/oneshim-client/pull/434))
  ...
  (linux)" with:
      ...
      resource path `maekon-sandbox-worker-x86_64-unknown-linux-gnu` doesn't exist

  The smoke script runs `cargo test -p maekon-app release_reliability_`
...
- Build oneshim-sandbox-worker for Tauri externalBin in release ([#430](https://github.com/pseudotop/oneshim-client/pull/430))
  ...
  macOS and (implicitly) other platforms with:
      ...
      resource path `maekon-sandbox-worker-x86_64-apple-darwin` doesn't exist

  Root cause: `tauri.conf.json` has `externalBin: ["maekon-sandbox-worker"]`
  ...
  step runs `cargo build -p maekon-app` which does not build the
  separate `maekon-sandbox-worker` workspace crate.
...
- Resolve CI lint failures ([#415](https://github.com/pseudotop/oneshim-client/pull/415))
  - Add maekon-sandbox-worker to arch-deps allowlist
...
- 6 layouts were suppressing <Outlet> on empty data ([#378](https://github.com/pseudotop/oneshim-client/pull/378))
  ...
  Regression test: crates/maekon-web/frontend/e2e/routing-empty-data.spec.ts
...
- Preset CRUD — PresetStorage port + V29 migration + API (X1 backend)
  ...
  - PresetStorage port trait in maekon-core (sync, follows FewShotStorage pattern)
...
- Remove maekon-web -> maekon-analysis cross-adapter dependency
  Move CoachingTemplate struct and static TEMPLATES data from
  maekon-analysis to maekon-core (where domain models belong).
...
- Mark 4K benchmark as #[ignore], update schema V25 in STATUS.md, add audio crate summary
  ...
  - CLAUDE.md: Add maekon-audio crate summary (capture, VAD, whisper, cloud STT)
...
- WebStorage trait split, integration.rs ADR-003, SyncTab rewrite, i18n sync, docs fix
  ...
  - CLAUDE.md: 13→14 crate count, remove stale suggestion→network
    exception, add maekon-audio to workspace diagram
...
- Add Bug Report Flow implementation plan (10 tasks)
  ...
  T1: BugId model + PiiSanitizer port (maekon-core)
  T2: VisionPiiSanitizer impl (maekon-vision)
  T3: Bug report DTOs (maekon-api-contracts)
  T4: BugReportService (maekon-web backend)
...
- Add FrameStoragePort abstraction (ADR-015)
  Define FrameStoragePort trait in maekon-core with 4 methods
  (save_frame, save_frames_batch, enforce_retention, enforce_storage_limit).
  Implement for FrameFileStorage in maekon-storage.
...
- Tech debt cleanup — remove unused async, idiomatic Option, match arms
  ...
  - Remove duplicate match arm in maekon-lint
...
- Add LLM-based WorkType classification refinement
  ...
  - Add lru dependency to maekon-analysis
...
- Integrate rectangle detection into element finder pipeline
  - Enable native-vision feature by default in maekon-vision
  ...
  - Forward native-vision feature from src-tauri to maekon-vision
...
- Filter MAEKON windows by app name + update README crate table
  Add app name filter ("MAEKON") before existing PID check in macOS
  ...
  to include maekon-analysis, maekon-embedding, maekon-api-contracts,
  maekon-lint, and removes deprecated maekon-app/maekon-ui entries.
...
- Enable programmatic resize for tracking panel expand/collapse
  ...
  Also adds MAEKON_AGENT=subagent skip condition to lefthook cargo-clippy
...
- Add tool definitions for maekon-web endpoints
  SessionContextAssembler.build_system_message() now populates the tools
  field with 7 key maekon-web REST API endpoint definitions (metrics,
...
- Deduplicate truncate_history, fix activity estimation, actual last_active
  - Extract truncate_chat_history() to maekon-core as shared utility,
...
- ADR-001 audit remediation — ports to core, async trait handlers ([#56](https://github.com/pseudotop/oneshim-client/pull/56))
  * refactor: ADR-001 audit remediation — ports to core, handlers to async traits
  ...
  - Define AuditLogPort and AutomationPort traits in maekon-core/src/ports/
  - Move GuiInteractionError to maekon-core::error
  - Move AuditEntry, AuditStatus, AuditLevel, AuditStats to maekon-core::models::audit
  - Move GuiExecutionResult to maekon-core::models::automation
  - Move GUI request types to maekon-core::models::gui
  - Move builtin_presets(), platform_modifier(), platform_alt_modifier() to maekon-core::models::intent
  ...
  - Replace all RwLock guard patterns in maekon-web handlers with direct port trait async calls
...
- Document Hexagonal Architecture violations in maekon-web (P4)
  maekon-web has adapter-to-adapter deps on maekon-storage (1 file,
  14 row types) and maekon-automation (7 files, 5 types). These are
  ...
  to maekon-core are implemented.
...
- GUI V2 M4 — End-to-End Workflow Tests (10 tests)
  ...
  - Added `async-trait` to maekon-web dev-dependencies (for mock impls)
...
- Migrate desktop runtime from iced to Tauri v2
  ...
  Key changes:
  - New src-tauri/ project replacing maekon-ui + maekon-app crates
...
- Add server/grpc feature flags — standalone build works without server deps
  - maekon-network is now optional (enabled via `server` or `grpc` feature)
...
- Replace server domain protos with Consumer Contract definitions
  - Delete 4 old server-domain generated files (maekon.v1.{auth,common,monitoring,user_context}.rs)
  - Add single client contract generated file (oneshim.client.v1.rs)
  - Update build.rs: compile 5 client protos from api/proto/maekon/client/v1/
...
- Remove unused maekon-network dep from maekon-suggestion
...
- Update crate documentation to reflect ADR-013 module splits
  ...
  - config.rs → config/ (maekon-core)
  - controller.rs, policy.rs → controller/, policy/ (maekon-automation)
  - scheduler.rs, updater.rs, focus_analyzer.rs → directory modules (maekon-app)
  - handlers/automation.rs → automation/ (maekon-web)
  - Test count: 821 → 831 (maekon-automation 183 → 193)
...
- V0.1.0
  First public release of MAEKON Rust desktop client.
