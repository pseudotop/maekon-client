...
pub(crate) fn register_debug_macos_notification_category(
    ...
    let action_identifier =
        NSString::from_str(super::debug_macos_notification_open_action_identifier());
    ...
    let category_identifier =
        NSString::from_str(super::debug_macos_notification_category_identifier());
...
pub(crate) mod debug_macos_notification_delegate {
    ...
    fn write_diagnostic_event(event: &'static str) {
        ...
        let payload = json!({
            ...
            "event": event,
            "route": route,
            "category_identifier": super::super::debug_macos_notification_category_identifier(),
            "open_action_identifier": super::super::debug_macos_notification_open_action_identifier(),
...
pub(crate) fn install_debug_macos_notification_delegate_from_env() {
    let activation_route = super::debug_notification_activation_route_from(
        std::env::var("MAEKON_DEBUG_NOTIFICATION_ACTIVATION_ROUTE")
            .ok()
            .as_deref(),
    );
    let activation_output_path = super::debug_notification_cli_activation_output_path_from(
        std::env::var("MAEKON_DEBUG_NOTIFICATION_ACTIVATION_OUTPUT")
            .ok()
            .as_deref(),
    );
    let diagnostic_jsonl_path = super::debug_notification_cli_diagnostic_jsonl_path_from(
