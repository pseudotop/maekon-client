...
use maekon_core::config::should_prompt;
...
mod tests {
    ...
    fn long_block_increments_counter() {
        ...
        mgr.update_with(|c| {
            c.autostart.prompt_state = maekon_core::config::AutostartPromptState::Pending;
            Ok(())
        })
        .expect("update");
    ...
    fn dismissed_state_skips_event_emission() {
        ...
        mgr.update_with(|c| {
            c.autostart.prompt_state = maekon_core::config::AutostartPromptState::Dismissed;
            Ok(())
        })
        .expect("update");
