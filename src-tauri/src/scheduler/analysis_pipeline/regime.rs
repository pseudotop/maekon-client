...
pub(in crate::scheduler) async fn run_periodic_regime_detection(
    ...
    match reader.get_entries(&window, true).await {
        Ok(entries) if !entries.is_empty() => {
            ...
            let features: Vec<maekon_core::models::tiered_memory::RegimeFeatures> = entries
                .iter()
                .map(|e| {
                    let cat = &e.app_category;
                    maekon_core::models::tiered_memory::RegimeFeatures {
...
async fn run_constrained_clustering(
    ts: &mut AdaptiveTriggerState,
    features: &[maekon_core::models::tiered_memory::RegimeFeatures],
    now: DateTime<Utc>,
) {
    use maekon_analysis::constraint_builder;
