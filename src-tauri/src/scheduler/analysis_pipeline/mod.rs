...
pub(super) async fn run_analysis_tick(
    ...
    let (work_type, engagement) = if let Some(ref content) = parsed_content {
        ...
        (
            maekon_core::models::tiered_memory::WorkType::Unknown,
            maekon_core::models::tiered_memory::EngagementMetrics::default(),
        )
    };
    ...
    let work_type = if let Some(fe) = focused_element {
        match fe.role.as_str() {
            ...
            "AXStaticText" | "text" => {
                ...
                match work_type {
                    maekon_core::models::tiered_memory::WorkType::ActiveCoding
                    | maekon_core::models::tiered_memory::WorkType::Writing
                    | maekon_core::models::tiered_memory::WorkType::DocumentWriting => {
                        if engagement.keystrokes_per_min < 5.0 {
                            maekon_core::models::tiered_memory::WorkType::Reading
                        } else {
                            work_type
                        }
                    }
    ...
    let terminal_command = focused_element
        ...
        .and_then(|text| {
            if app_subcategory == AppSubcategory::Terminal {
                maekon_analysis::terminal_detector::detect_terminal_command(text)
            } else {
                None
            }
        });
    ...
    let doc_heading = focused_element
        ...
        .and_then(|text| {
            if app_subcategory == AppSubcategory::DocumentEditor {
                maekon_analysis::document_heading::extract_document_heading(text)
            } else {
                None
            }
        });
