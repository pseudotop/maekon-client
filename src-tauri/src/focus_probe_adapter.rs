...
use maekon_core::models::gui::{ExecutionBinding, FocusSnapshot, FocusValidation};
...
mod tests {
    ...
    impl ProcessMonitor for MockProcessMonitor {
        ...
        async fn get_top_processes(
            &self,
            _limit: usize,
        ) -> Result<Vec<maekon_core::models::context::ProcessInfo>, CoreError> {
            Ok(vec![])
        }
        ...
        async fn get_detailed_processes(
            ...
            _top_n: usize,
        ) -> Result<Vec<maekon_core::models::event::ProcessDetail>, CoreError> {
            Ok(vec![])
        }
    }
