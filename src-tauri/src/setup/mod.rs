...
pub(crate) mod ext_grpc_handles;
...
mod tests {
    ...
    /// Verifies that `run()` (in lib.rs — #7734 moved the Tauri builder body
    /// here from main.rs) has a RunEvent::Reopen handler.
    ...
    fn main_contains_reopen_handler() {
        ...
        assert!(
            ...
            "lib.rs's run() must handle RunEvent::Reopen for macOS dock icon clicks"
        );
    }
}
