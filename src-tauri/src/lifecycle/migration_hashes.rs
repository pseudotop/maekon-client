...
//! `~/.config/systemd/user/maekon.service` file matches a known template
...
pub const KNOWN_PRIOR_HASHES: &[(&str, &str)] = &[
    ...
    //
    // Retained as the canonical PR-B1 → PR-B2 example; future PR-Bn migrations
    // append additional entries here. The pre-rebrand ONESHIM-branded variant
    // (v0.4.40-rc.1/rc.2) was dropped during the v0.0.1 brand reset since
    // those releases only shipped on the archived `pseudotop/oneshim-client`
    // legacy repo and have no active install base on the maekon-client line.
    (
