...
fn build_controller_from_runtime(
    ...
    let overlay_driver: Arc<dyn maekon_core::ports::overlay_driver::OverlayDriver> =
...
fn preflight_provider_oauth_connection(
    ...
    oauth_port: Option<Arc<dyn OAuthPort>>,
) -> std::result::Result<Option<Arc<dyn OAuthPort>>, maekon_core::error::CoreError> {
    ...
    let oauth = oauth_port.ok_or_else(|| maekon_core::error::CoreError::Config {
    ...
    for provider_id in selected_provider_ids {
        let status = handle
            ...
            .map_err(|e| maekon_core::error::CoreError::Config {
                code: maekon_core::error_codes::ConfigCode::Invalid,
                message: e.to_string(),
            })?;

        if !status.connected && !status.has_refresh_token {
            return Err(maekon_core::error::CoreError::Config { code: maekon_core::error_codes::ConfigCode::Invalid, message: format!(
...
fn oauth_runtime_error_status(ai_config: &AiProviderConfig, reason: String) -> AiRuntimeStatus {
    let ocr_source = match ai_config.ocr_provider {
        maekon_core::config::OcrProviderType::Remote => "remote",
        maekon_core::config::OcrProviderType::Local => "local",
    };
