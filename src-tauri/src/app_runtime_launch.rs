    ...
    pub(crate) fn build_and_spawn(self) -> Result<AppRuntimeLaunchResult> {
        ...
        let suggestion_manager: Option<Arc<crate::suggestion_manager::SuggestionManager>> = {
            ...
            let api_result: Result<
                Arc<dyn maekon_core::ports::api_client::ApiClient>,
                _,
            > = {
            ...
            match api_result {
                Ok(api) => {
                    ...
                    let feedback =
                        Arc::new(maekon_suggestion::feedback::FeedbackSender::new_with_sink(
                            api,
        ...
        if let Some(ref mgr) = suggestion_manager {
            ...
            if !pending_feedbacks.is_empty() {
                ...
                for record in pending_feedbacks {
                    ...
                    {
                        rq.enqueue(maekon_suggestion::feedback_retry::PendingFeedback {
                            suggestion_id: sid,
                            feedback_type: ft,
                            comment,
                            attempts,
        ...
        let session_manager = {
            ...
            let audit_port: Arc<dyn maekon_core::ports::audit_log::AuditLogPort> =
                Arc::new(maekon_automation::audit::AuditLogAdapter::new(audit_logger));
        ...
        let automation_controller = if config.web.enabled {
            ...
            .with_coaching_engine(
                coaching_engine.clone() as Arc<dyn maekon_core::ports::coaching::CoachingPort>
            );
            if let Some((ref sm, _)) = session_manager {
                builder = builder
                    .with_session_manager(sm.clone()
                        as Arc<dyn maekon_core::ports::conversation_session::SessionManager>);
            }
            if let Some(ref capture_services) = shared_capture_services {
            ...
            let (ext_shared_live, ext_shared_metrics) = {
                if config.external_grpc.enabled {
                    ...
                    let metrics = std::sync::Arc::new(
                        maekon_web::grpc::external::metrics::ExternalMetrics::new(),
                    );
            ...
            //   1. MAEKON_DASHBOARD_GRPC_PORT env var (ops/CI override)
            //   2. `config.web.grpc_port` (user config)
            //   3. `maekon_web::grpc::DEFAULT_GRPC_DASHBOARD_PORT` (10080)
            // If the default/configured port is busy, maekon-web tries the
            ...
            #[cfg(feature = "grpc-dashboard")]
            {
                ...
                let grpc_storage = sqlite_storage.clone()
                    as std::sync::Arc<dyn maekon_web::storage_port::WebStorage>;
                ...
                let grpc_monitor = std::sync::Arc::new(maekon_monitor::system::SysInfoMonitor::new())
                    as std::sync::Arc<dyn maekon_core::ports::monitor::SystemMonitor>;
                let thresholds = config.web.grpc_load_thresholds.clone().unwrap_or_default();
                let load_policy =
                    std::sync::Arc::new(maekon_web::grpc::LoadPolicy::new(thresholds));
            ...
            #[cfg(feature = "grpc-dashboard-external")]
            {
                ...
                if ext_cfg.enabled {
                    ...
                    if let Err(msg) =
                        maekon_web::grpc::external::port_collision::check_port_collision(
                            ext_cfg.port,
                            loopback_port,
                        )
                    {
                        ...
                        let ext_audit: std::sync::Arc<
                            dyn maekon_core::ports::audit_log::AuditLogPort,
                        > = {
                            let storage_for_audit = sqlite_storage.clone();
                            let persistence_cb: std::sync::Arc<
                                dyn maekon_automation::audit::AuditPersistence,
                            > = std::sync::Arc::new(
                                move |entry: &maekon_core::models::audit::AuditEntry| {
                                    storage_for_audit.save_audit_entry(entry);
                                },
                            );
                            let audit_query: std::sync::Arc<
                                dyn maekon_automation::audit::AuditQuery,
                            ...
                            std::sync::Arc::new(maekon_automation::audit::AuditLogAdapter::new(
                                logger,
                            ))
                        };
                        ...
                        match handle.block_on(build_external_spawn_config(
                            ...
                            Ok(spawn_cfg) => {
                                let _ext_handle = handle.block_on(
                                    maekon_web::grpc::external::spawn_with_supervisor(spawn_cfg),
                                );
                                tracing::info!(
                ...
                let cloud_provider: Option<Arc<dyn maekon_core::ports::stt_provider::SttProvider>> = {
                    #[cfg(feature = "cloud-stt")]
                    {
                        if !config.audio.cloud_api_key.is_empty() {
                            match maekon_audio::CloudSttProvider::new(
        ...
        let model_downloader: Option<
            Arc<dyn maekon_core::ports::model_downloader::ModelDownloader>,
        > = {
            #[cfg(feature = "download")]
            {
...
) -> anyhow::Result<maekon_web::grpc::external::spawn_config::ExternalGrpcSpawnConfig> {
