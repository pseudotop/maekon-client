    ...
    if config.analysis.embedding.enabled {
        ...
        > = match embedding_config.provider {
            ...
            maekon_core::config::EmbeddingProviderType::Local => {
                match maekon_embedding::LocalEmbeddingProvider::new() {
                    Ok(provider) => {
            ...
            maekon_core::config::EmbeddingProviderType::Remote => {
                if let Some(ref endpoint) = embedding_config.remote_endpoint {
                    let api_key = config
                        ...
                        .unwrap_or_default();
                    Some(Arc::new(
                        maekon_network::remote_embedding_client::RemoteEmbeddingProvider::new(
                            endpoint.clone(),
                            api_key,
            ...
            if embedding_config.llm_summary_enabled {
                if let Some(ref _llm_api) = config.ai_provider.llm_api {
                    let analysis_provider: Arc<
                        dyn maekon_core::ports::analysis_provider::AnalysisProvider,
