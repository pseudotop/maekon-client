...
pub const MAEKON_PROVIDER_SECRET_BACKEND_ENV: &str = "MAEKON_PROVIDER_SECRET_BACKEND";
pub const MAEKON_PROVIDER_SECRET_FILE_ENV: &str = "MAEKON_PROVIDER_SECRET_FILE";
pub const FILE_SECRET_STORE_NAME: &str = "maekon-secrets.json";
...
fn requested_provider_secret_backend() -> Result<RequestedProviderSecretBackend, CoreError> {
    let Some(raw) = std::env::var(MAEKON_PROVIDER_SECRET_BACKEND_ENV).ok() else {
    ...
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        other => Err(CoreError::Config { code: maekon_core::error_codes::ConfigCode::Invalid, message: format!(
...
fn file_secret_store_path(config_dir: &Path) -> PathBuf {
    std::env::var_os(MAEKON_PROVIDER_SECRET_FILE_ENV)
        .map(PathBuf::from)
...
mod tests {
    use super::*;
    use maekon_core::config::{CredentialAuthMode, SecretRef};
