...
use maekon_core::config::{CredentialAuthMode, CredentialBackendKind, ExternalApiEndpoint};
...
use maekon_core::provider_surface::provider_vendor_id_or_default;
use maekon_storage::process_env_projection::{
    provider_api_key_cli_template, ProcessEnvSecretProjection,
};
use maekon_storage::temp_file_projection::{
...
pub fn run(args: &[String], config_dir: &Path) -> i32 {
    match args.first().map(String::as_str) {
        ...
        _ => {
            eprintln!("Usage: maekon secret <status|env|file|exec> ...");
            eprintln!();
            eprintln!("Commands:");
            eprintln!(
...
fn cmd_status(args: &[String], config_dir: &Path) -> i32 {
    let Some(surface) = args.first().and_then(|value| SecretSurface::parse(value)) else {
        eprintln!("Usage: maekon secret status <llm|ocr>");
        return 1;
    };
...
fn cmd_env(args: &[String], config_dir: &Path) -> i32 {
    let Some(surface) = args.first().and_then(|value| SecretSurface::parse(value)) else {
        eprintln!("Usage: maekon secret env <llm|ocr>");
        return 1;
    };
...
fn cmd_file(args: &[String], config_dir: &Path) -> i32 {
    let Some(surface) = args.first().and_then(|value| SecretSurface::parse(value)) else {
        eprintln!("Usage: maekon secret file <llm|ocr>");
        return 1;
    };
...
fn cmd_exec(args: &[String], config_dir: &Path) -> i32 {
    let Ok((surface, command)) = parse_exec_args(args) else {
        eprintln!("Usage: maekon secret exec <llm|ocr> -- <command...>");
        return 1;
    };
...
fn endpoint_for_surface(
    config: &maekon_core::config::AiProviderConfig,
...
fn resolve_env_projection(
    ...
    if let (Some(secret_store), Some(secret_ref)) = (
        ...
        let request = SecretProjectionRequest {
            ...
            key: secret_ref.key,
            target: maekon_core::ports::secret_projection::ProjectionTarget::ProcessEnv,
        ...
        match binding.backend_kind {
            CredentialBackendKind::OsSecretStore | CredentialBackendKind::FileSecretStore => {
                ...
                let request = SecretProjectionRequest {
                    ...
                    target: maekon_core::ports::secret_projection::ProjectionTarget::TempFile,
                    purpose: ProjectionPurpose::ProviderCliExecution,
                    consumer_id: template.consumer_id.clone(),
                };
                let projection =
