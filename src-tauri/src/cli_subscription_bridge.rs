...
const MAEKON_BRIDGE_NAME: &str = "maekon-context";
const MAEKON_MANAGED_BRIDGE_MARKER: &str = "MAEKON-MANAGED-CLI-BRIDGE: maekon-context";
const MAEKON_CLI_BRIDGE_AUTOINSTALL_ENV: &str = "MAEKON_CLI_BRIDGE_AUTOINSTALL";
const MAEKON_CLI_BRIDGE_INCLUDE_USER_SCOPE_ENV: &str = "MAEKON_CLI_BRIDGE_INCLUDE_USER_SCOPE";
...
pub fn default_context_export_path(data_dir: &Path) -> PathBuf {
    data_dir.join("exports").join("maekon-context.json")
}
...
fn bridge_file_path(base_dir: &Path, client: CliClient) -> PathBuf {
    match client {
        CliClient::Codex => base_dir
            .join(".codex")
            .join("commands")
            .join(format!("{MAEKON_BRIDGE_NAME}.md")),
        ...
        CliClient::GeminiCli => base_dir
            .join(".gemini")
            .join("commands")
            .join(format!("{MAEKON_BRIDGE_NAME}.toml")),
    }
}
...
"#,
        marker = MAEKON_MANAGED_BRIDGE_MARKER,
...
pub fn should_autoinstall_bridge_files() -> bool {
    env_flag_enabled(MAEKON_CLI_BRIDGE_AUTOINSTALL_ENV)
}

/// Whether user-scope bridge files (`~/.codex`, `~/.claude`, `~/.gemini`) are included.
pub fn should_include_user_scope() -> bool {
    env_flag_enabled(MAEKON_CLI_BRIDGE_INCLUDE_USER_SCOPE_ENV)
}
...
fn is_managed_bridge_content(contents: &str) -> bool {
    if contents.contains(MAEKON_MANAGED_BRIDGE_MARKER) {
        return true;
    }
...
mod tests {
    ...
    fn default_bridge_plans_include_project_plans_for_all_clients() {
        let project = PathBuf::from("/tmp/maekon-project");
    ...
    fn render_markdown_template_includes_context_path() {
        let plan = CliBridgePlan {
            ...
            file_path: PathBuf::from("/tmp/.codex/commands/maekon-context.md"),
        };
        let context_path = PathBuf::from("/tmp/context/export.json");
        let rendered = render_bridge_template(&plan, &context_path);

        assert!(rendered.contains(MAEKON_MANAGED_BRIDGE_MARKER));
    ...
    fn render_gemini_template_is_toml() {
        let plan = CliBridgePlan {
            ...
            file_path: PathBuf::from("/tmp/.gemini/commands/maekon-context.toml"),
        };
        let context_path = PathBuf::from("/tmp/context/export.json");
        let rendered = render_bridge_template(&plan, &context_path);
    ...
    fn materialize_bridge_file_is_idempotent() {
        ...
        let plan = CliBridgePlan {
            ...
            scope: BridgeScope::Project,
            artifact: BridgeArtifactKind::MarkdownCommand,
            file_path: temp.path().join(".codex/commands/maekon-context.md"),
        };
