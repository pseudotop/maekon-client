...
fn source_label(source: &maekon_core::models::suggestion::SuggestionSource) -> &'static str {
    match source {
        maekon_core::models::suggestion::SuggestionSource::LlmServer => "server",
        maekon_core::models::suggestion::SuggestionSource::LlmLocal => "local",
        maekon_core::models::suggestion::SuggestionSource::RuleBased => "rule",
    }
}
...
pub async fn submit_suggestion_feedback(
    ...
    if let Some(suggestion) = removed {
        let feedback_type = match action.as_str() {
            "accept" => maekon_core::models::suggestion::FeedbackType::Accepted,
            "reject" => maekon_core::models::suggestion::FeedbackType::Rejected,
            _ => unreachable!(),
        };
        mgr.scorer().lock().await.record(
