...
//!
//! ```ignore
//! // maekon_network::NetworkError -> CoreError -> IpcError
...
/// `crates/maekon-core/tests/wire_contract_snapshot.expected.txt`). Frontend
...
impl From<maekon_storage::error::StorageError> for IpcError {
    fn from(err: maekon_storage::error::StorageError) -> Self {
        IpcError::from(CoreError::from(err))
    }
}

impl From<maekon_automation::error::AutomationError> for IpcError {
    fn from(err: maekon_automation::error::AutomationError) -> Self {
        IpcError::from(CoreError::from(err))
    }
}

impl From<maekon_vision::error::VisionError> for IpcError {
    fn from(err: maekon_vision::error::VisionError) -> Self {
        IpcError::from(CoreError::from(err))
    }
}

impl From<maekon_analysis::error::AnalysisError> for IpcError {
    fn from(err: maekon_analysis::error::AnalysisError) -> Self {
        IpcError::from(CoreError::from(err))
    }
}

impl From<maekon_suggestion::error::SuggestionError> for IpcError {
    fn from(err: maekon_suggestion::error::SuggestionError) -> Self {
        IpcError::from(CoreError::from(err))
    }
}

impl From<maekon_monitor::error::MonitorError> for IpcError {
    fn from(err: maekon_monitor::error::MonitorError) -> Self {
        IpcError::from(CoreError::from(err))
    }
}
...
impl From<maekon_network::error::NetworkError> for IpcError {
    fn from(err: maekon_network::error::NetworkError) -> Self {
        IpcError::from(CoreError::from(err))
    }
}

#[cfg(feature = "embedding")]
impl From<maekon_embedding::error::EmbeddingError> for IpcError {
    fn from(err: maekon_embedding::error::EmbeddingError) -> Self {
        IpcError::from(CoreError::from(err))
    }
}
...
mod tests {
    ...
    fn ipc_error_from_adapter_error_chains_through_core() {
        ...
        let net = maekon_network::error::NetworkError::Timeout { timeout_ms: 3000 };
    ...
    fn ipc_error_from_adapter_error_storage_variant() {
        let storage = maekon_storage::error::StorageError::NotFound {
