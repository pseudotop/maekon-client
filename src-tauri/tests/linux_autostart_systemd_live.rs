...
//!
//! Run manually:
//!     cargo test -p maekon-app --features systemd-notify \
...
//! - T8 / T10 modify ~/.config/systemd/user/maekon.service
...
fn enable_then_disable_writes_type_notify_service_file() {
    ...
    if service_path.exists() {
        ...
        eprintln!("SKIP: maekon binary not built or test environment lacks autostart write perms");
    }
}
...
fn migration_writes_type_notify_when_pr_b1_template_present() {
    ...
    //   2. Toggle autostart ON in Settings → verify ~/.config/systemd/user/maekon.service has Type=simple
    ...
    //   7. Verify systemctl --user is-active maekon.service still returns "active"
