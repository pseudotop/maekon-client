        ...
        let info = match effective_level {
            ...
            PiiFilterLevel::Basic => FocusedElementInfo {
                ...
                extracted_text: self.value.as_ref().map(|v| {
                    maekon_vision::privacy::sanitize_title_with_level(v, PiiFilterLevel::Basic)
                }),
            },
...
fn accessibility_disabled_when_config_false() {
    use maekon_core::config::TextIntelligenceConfig;
...
fn create_extractor_returns_expected_on_platform() {
    let extractor = maekon_vision::accessibility::create_extractor();
...
fn macos_extractor_reports_correct_name() {
    let extractor = maekon_vision::accessibility::create_extractor().unwrap();
