...
fn terminal_command_detected_from_basic_pii_text() {
    ...
    let cmd = maekon_analysis::terminal_detector::detect_terminal_command(
...
fn document_heading_detected_from_editor_text() {
    ...
    let heading = maekon_analysis::document_heading::extract_document_heading(
        element.extracted_text.as_deref().unwrap(),
    );
...
fn pii_basic_level_masks_email_preserves_command() {
    ...
    let filtered =
        maekon_vision::privacy::sanitize_title_with_level(raw_text, PiiFilterLevel::Basic);
    ...
    let cmd = maekon_analysis::terminal_detector::detect_terminal_command(&filtered);
    assert!(cmd.is_some());
...
fn context_assembler_includes_accessibility_text() {
    use maekon_analysis::{ContextAssembler, CurrentActivity, SessionMetrics};
...
fn full_pipeline_terminal_basic_level() {
    ...
    let filtered = maekon_vision::privacy::sanitize_title_with_level(raw, PiiFilterLevel::Basic);
...
fn full_pipeline_document_heading_off_level() {
    ...
    let text = maekon_vision::privacy::sanitize_title_with_level(raw, PiiFilterLevel::Off);

    let heading = maekon_analysis::document_heading::extract_document_heading(&text);
    assert!(heading.is_some());
    let heading = heading.unwrap();
...
fn terminal_detection_multiple_prompts() {
    // zsh percent prompt
    let cmd = maekon_analysis::terminal_detector::detect_terminal_command("% npm run build");
    assert!(cmd.is_some());
    assert_eq!(cmd.unwrap().command, "npm");

    // PowerShell chevron
    let cmd = maekon_analysis::terminal_detector::detect_terminal_command("> Get-Process");
    assert!(cmd.is_some());
    assert_eq!(cmd.unwrap().command, "Get-Process");

    // root hash prompt
    let cmd =
        maekon_analysis::terminal_detector::detect_terminal_command("# systemctl restart nginx");
    assert!(cmd.is_some());
