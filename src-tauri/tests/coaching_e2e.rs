...
use maekon_analysis::coaching_engine::CoachingEngine;
use maekon_core::config::{CoachingConfig, ProfileConfig, TimeRange};
use maekon_core::models::coaching::TriggerType;
...
async fn e2e_feedback_tracking() {
    ...
    let trigger_name = maekon_core::models::coaching::trigger_type_name(&m.trigger);
    engine
