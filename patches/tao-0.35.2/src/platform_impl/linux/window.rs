...
impl Window {
    pub(crate) fn new<T>(
        ...
        let mut window_builder = gtk::ApplicationWindow::builder()
            .application(app)
            .can_focus(attributes.focusable && attributes.focused);
        ...
        if let Some(position) = attributes.position {
            ...
            let _ = (x, y);
            log::warn!("GTK4 does not support initial toplevel window positioning");
        }

        if pl_attribs.rgba_visual || attributes.transparent {
            log::warn!("GTK4 no longer exposes per-window RGBA visual selection");
        }

        if pl_attribs.app_paintable || attributes.transparent {
            log::warn!("GTK4 removed GtkWidget app-paintable window setup");
        }

        if !pl_attribs.double_buffered {
            log::warn!("GTK4 removed explicit double-buffer configuration");
        }
        ...
        if attributes.visible_on_all_workspaces {
            log::warn!("GTK4 does not support visible-on-all-workspaces window hints");
        }
        ...
        // restore accept-focus after the window has been mapped
        ...
        if attributes.focusable && !attributes.focused {
            ...
            let id = window.connect_map(move |window| {
    ...
    fn setup_signals(
        ...
        let (w, h) = Self::current_window_size(window);
        let inner_position: Rc<(AtomicI32, AtomicI32)> =
            Rc::new((AtomicI32::new(0), AtomicI32::new(0)));
        let outer_position: Rc<(AtomicI32, AtomicI32)> =
            Rc::new((AtomicI32::new(0), AtomicI32::new(0)));
        let inner_size: Rc<(AtomicI32, AtomicI32)> =
            Rc::new((AtomicI32::new(w), AtomicI32::new(h)));
        let outer_size: Rc<(AtomicI32, AtomicI32)> =
            Rc::new((AtomicI32::new(w), AtomicI32::new(h)));
        ...
        window.connect_default_width_notify(move |window| {
            let (w, h) = Self::current_window_size(window);
            inner_size_clone.0.store(w, Ordering::Release);
            inner_size_clone.1.store(h, Ordering::Release);
        ...
        window.connect_maximized_notify(move |window| {
            max_clone.store(window.is_maximized(), Ordering::Release);
        });
    ...
    fn current_window_size(window: &gtk::ApplicationWindow) -> (i32, i32) {
    ...
    pub fn current_monitor(&self) -> Option<RootMonitorHandle> {
        let display = widget_display(&self.window);
        ...
        self.surface()
            ...
            .map(|monitor| RootMonitorHandle {
                inner: MonitorHandle { monitor },
            })
            .or_else(|| monitor::primary(&display).map(|inner| RootMonitorHandle { inner }))
    }

    #[inline]
    pub fn available_monitors(&self) -> VecDeque<MonitorHandle> {
        monitor::available(&widget_display(&self.window))
    }

    pub fn primary_monitor(&self) -> Option<RootMonitorHandle> {
        monitor::primary(&widget_display(&self.window)).map(|inner| RootMonitorHandle { inner })
    }
    ...
    fn is_wayland(&self) -> bool {
        widget_display(&self.window).backend().is_wayland()
    }

    fn surface(&self) -> Option<gtk::gdk::Surface> {
        self.window.surface()
    }
    ...
    pub fn raw_window_handle_rwh_04(&self) -> rwh_04::RawWindowHandle {
        if self.is_wayland() {
            ...
            #[cfg(feature = "x11")]
            if let Some(surface) = self.surface().and_then(|s| s.downcast::<X11Surface>().ok()) {
                window_handle.window = surface.xid();
            }
        ...
        if self.is_wayland() {
            ...
            display_handle.display = unsafe {
                gdk_wayland_sys::gdk_wayland_display_get_wl_display(
                    widget_display(&self.window).as_ptr() as *mut _,
                )
            };
    ...
    pub fn raw_window_handle_rwh_06(&self) -> Result<rwh_06::RawWindowHandle, rwh_06::HandleError> {
        if self.is_wayland() {
            let surface = self.surface().ok_or(rwh_06::HandleError::Unavailable)?;
            let surface = unsafe {
                gdk_wayland_sys::gdk_wayland_surface_get_wl_surface(surface.as_ptr() as *mut _)
            };
            let surface =
                std::ptr::NonNull::new(surface).ok_or(rwh_06::HandleError::Unavailable)?;
            ...
            #[cfg(feature = "x11")]
            {
                ...
                let window_handle = rwh_06::XlibWindowHandle::new(surface.xid());
...
fn widget_display<W: IsA<gtk::Widget>>(window: &W) -> gtk::gdk::Display {
