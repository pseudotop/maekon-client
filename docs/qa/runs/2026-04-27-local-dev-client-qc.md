...
- Bundle identifier: `com.maekon.app.dev`
...
- The release app (`com.maekon.app`) was not used for this pass. The running app was `Maekon Dev` / `com.maekon.app.dev`.
...
1. `cargo tauri dev` can lead QC toward the installed release app identity. Use the built debug bundle (`target/debug/bundle/macos/Maekon Dev.app`, bundle id `com.maekon.app.dev`) for native QC to avoid release/dev confusion.
...
- Debug bundle guardrail was added to `scripts/build-macos-dev-bundle.sh`; it now verifies `com.maekon.app.dev` and `Maekon Dev` before reporting the launch command.
...
- Bundle identifier is `com.maekon.app.dev`.
