...
- [ ] `maekon-core` remains the contract layer. No delivery/runtime logic was pushed into domain crates.
- [ ] Adapter crates still implement `maekon-core` ports instead of introducing direct adapter-to-adapter coupling.
- [ ] `maekon-app` remains the composition root and runtime orchestrator.
- [ ] `maekon-web` remains a delivery layer. Handlers stay thin and service/context boundaries remain explicit.
...
- [ ] `cargo test -p maekon-network --test integration_fake_server -- --nocapture`
- [ ] `cargo test -p maekon-web integration --lib -- --nocapture`
- [ ] `cargo test -p maekon-web settings_service --lib -- --nocapture`
- [ ] `cargo test -p maekon-web ai_model_catalog_service --lib -- --nocapture`
- [ ] `cargo test -p maekon-app --features server build_integration_runtime -- --nocapture`
- [ ] `cargo check --workspace`
- [ ] `cargo clippy -p maekon-network --all-targets -- -D warnings`
- [ ] `cargo clippy -p maekon-web --all-targets -- -D warnings`
- [ ] `cargo clippy -p maekon-app --all-targets -- -D warnings`
- [ ] `cargo fmt --all --check`
- [ ] `git diff --check`
