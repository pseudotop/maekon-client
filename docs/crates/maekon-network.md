[English](./maekon-network.md) | [한국어](./maekon-network.ko.md)
...
Two clients that call external AI APIs. Both branch by provider using the `AiProviderType` enum from `maekon-core`.
Versioned contract reference: `docs/contracts/ai-provider-contract.md`.

```rust
// maekon-core/config.rs
...
```rust
use maekon_network::{TokenManager, HttpApiClient, SseStreamClient};
