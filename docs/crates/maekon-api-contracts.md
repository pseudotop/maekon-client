...
`maekon-api-contracts` centralizes transport-layer DTOs for the local web API.
...
- OpenAPI standardization bridge via interface manifest:
  ...
  - `docs/contracts/maekon-web.v1.openapi.yaml`
...
4. Treat this crate as the transport SSOT for `maekon-web` APIs.
5. Keep `maekon-web/src/handlers` free of public DTO definitions (enforced by `scripts/verify-web-contract-boundary.sh`).
