[English](./maekon-app.md) | [한국어](./maekon-app.ko.md)
...
> **Note**: the `maekon-app` package now lives at `src-tauri/` (composition root) per [ADR-004](../architecture/ADR-004-tauri-v2-migration.md) Tauri v2 migration. The former `crates/maekon-app/` directory was removed from the workspace.

```
src-tauri/src/  (package: maekon-app)
...
- Enable auto-install: `MAEKON_CLI_BRIDGE_AUTOINSTALL=1`
- Include user-scope directories (`~/.codex`, `~/.claude`, `~/.gemini`): `MAEKON_CLI_BRIDGE_INCLUDE_USER_SCOPE=1`
- Default context export reference path in generated bridge files: `<data_dir>/exports/maekon-context.json`
...
impl Autostart {
    ...
    pub fn enable() -> Result<(), CoreError> {
        ...
        let path = dirs::home_dir()
            .unwrap()
            .join("Library/LaunchAgents/com.maekon.app.plist");
    ...
    pub fn enable() -> Result<(), CoreError> {
        ...
        unsafe {
            ...
            RegSetValueExW(hkey, "MAEKON", /* exe_path */)?;
            RegCloseKey(hkey);
        }
        Ok(())
    }
