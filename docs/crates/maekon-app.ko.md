...
> **참고**: `maekon-app` 패키지는 [ADR-004](../architecture/ADR-004-tauri-v2-migration.ko.md) Tauri v2 마이그레이션에 따라 이제 `src-tauri/` (composition root) 에 위치합니다. 구 `crates/maekon-app/` 디렉토리는 워크스페이스에서 제거되었습니다.

```
src-tauri/src/  (패키지: maekon-app)
...
- 자동 설치 활성화: `MAEKON_CLI_BRIDGE_AUTOINSTALL=1`
- 사용자 스코프 디렉토리(`~/.codex`, `~/.claude`, `~/.gemini`) 포함: `MAEKON_CLI_BRIDGE_INCLUDE_USER_SCOPE=1`
- 생성되는 브리지 파일의 기본 컨텍스트 경로: `<data_dir>/exports/maekon-context.json`
