...
**상태**: Accepted
**날짜**: 2026-04-17
**범위**: `maekon-core::config_manager::ConfigManager`, 런타임에서 `AppConfig`를 읽는 모든 소비자

---

## 컨텍스트

이전까지 `ConfigManager`(`maekon-core`)는 `Arc<RwLock<AppConfig>>`를 보유하고
...
구현했고, 일부 소비자(`maekon-vision::privacy`,
`maekon-analysis::regime_manager`)는 init 시점에 섹션을 캐시하여 이후 변경을
...
`src-tauri`, `maekon-web`, 스케줄러 루프에 걸친 20개 이상의 기존 호출 지점이
수정 없이 호환된다.
