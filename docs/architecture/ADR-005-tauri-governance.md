...
The previous GUI was implemented with the `iced` immediate-mode GUI library (crate `maekon-ui`). This was replaced in ADR-004.
...
- Single UI codebase (React). The existing `crates/maekon-web/frontend/` is reused without modification.
