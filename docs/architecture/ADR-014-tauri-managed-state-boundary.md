...
`maekon-core` ports rather than through concrete adapter types.

However, `maekon-app` also owns the desktop entry point and Tauri framework boundary. That creates
...
3. Leaving raw implementation types in `AppState` makes the command boundary depend on internal
   service shapes. Pushing every desktop-only helper method into `maekon-core` would instead
...
- `maekon-core` ports grow delivery-specific and runtime-specific methods only to satisfy Tauri.
...
`maekon-app` remains the single composition root for the desktop binary.
...
1. `Arc<dyn PortTrait>` from `maekon-core` when the capability is a real cross-crate contract.
...
1. Use a `maekon-core` port when the capability expresses a stable business or application
...
- domain/application contracts belong in `maekon-core`
...
### 6. Do Not Pollute `maekon-core` to Satisfy a Framework

`maekon-core` ports MUST NOT gain methods solely because a Tauri command wants a convenience API.
...
`maekon-core` intentionally. They should not start there by default.
...
### A. Put Every Needed Method on `maekon-core` Ports
...
1. `maekon-core` stays focused on reusable contracts instead of Tauri convenience APIs.
...
3. Does the change avoid adding framework-only convenience methods to `maekon-core`?
