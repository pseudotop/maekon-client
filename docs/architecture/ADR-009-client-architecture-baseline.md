...
- `maekon-app` composition-root structure
...
1. `maekon-core` is the domain contract layer.
2. Adapter crates implement ports from `maekon-core`.
3. `maekon-app` is the composition root and runtime orchestrator.
4. `maekon-web` is a delivery layer only.
...
3. `WebContext` definitions live in [web_contexts/mod.rs](../../crates/maekon-web/src/services/web_contexts/mod.rs).
...
4. letting web-only delivery concerns leak into `maekon-core`
...
3. `maekon-web` is significantly easier to review because handlers, contexts, services, and assemblers have clearer roles.
4. `maekon-app` is less likely to regress into an oversized composition root.
...
2. Does it keep `maekon-web` as a delivery layer?
