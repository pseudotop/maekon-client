...
**상태**: Accepted
**날짜**: 2026-04-03
**범위**: `maekon-core` 포트, `maekon-storage` 어댑터, `src-tauri` 컴포지션 루트

---

## 컨텍스트

`FrameFileStorage`는 `maekon-storage`의 구체 타입으로, 프레임 이미지 저장
...
`maekon-core::ports`에 `FrameStoragePort` trait를 도입하여 소비자가 실제로
...
- 의존성 그래프 명확화 — `maekon-storage`는 와이어링 코드에서만 참조
