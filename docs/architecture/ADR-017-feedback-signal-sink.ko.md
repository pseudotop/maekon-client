...
**날짜 (Date)**: 2026-04-18
**범위 (Scope)**: `maekon-core::ports::feedback_signal_sink`, `maekon-suggestion::FeedbackSender`, `maekon-analysis::CoachingEngine/RegimeClassifier`, `src-tauri::feedback_sink::CompositeFeedbackSink`
...
`maekon-core::ports::feedback_signal_sink` 에 새 포트 `FeedbackSignalSink` 를 추가한다:
...
- `FeedbackSender` 가 `Arc<CoachingEngine>` 을 직접 참조 — 기각. 헥사고날 경계 위반(`maekon-suggestion` 이 `maekon-analysis` 에 의존하게 된다).
