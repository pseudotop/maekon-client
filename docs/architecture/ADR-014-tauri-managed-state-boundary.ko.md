...
ADR-001은 cross-crate 동작이 concrete adapter 타입이 아니라 `maekon-core` port를 통해
흘러야 한다는 기본 원칙을 올바르게 정한다.

하지만 `maekon-app`은 데스크톱 엔트리포인트이자 Tauri 프레임워크 경계도 함께 소유한다.
...
`maekon-app`은 데스크톱 바이너리의 단일 composition root로 남는다.
...
1. 실제 cross-crate contract를 표현하는 경우 `maekon-core`의 `Arc<dyn PortTrait>`
...
- domain/application contract는 `maekon-core`
...
### 6. Framework를 만족시키기 위해 `maekon-core`를 오염시키지 않는다

Tauri command가 convenience API를 원한다는 이유만으로 `maekon-core` port에 메서드를
추가해서는 안 된다.
...
이 연산들이 나중에 실제 multi-crate contract가 되면 그때 의도적으로 `maekon-core`로 올린다.
처음부터 거기에 둘 필요는 없다.
...
### A. 필요한 메서드를 전부 `maekon-core` port에 넣는다

기본안으로는 기각한다.

이 방식은 command 코드를 모두 trait 기반으로 유지하지만, Tauri 전용 lifecycle과 convenience
연산까지 core로 밀어 넣게 된다. 그러면 `maekon-core`의 의미가 약해진다.
...
1. `maekon-core`가 Tauri convenience API가 아니라 재사용 가능한 contract에 집중할 수 있다.
...
3. Framework-only convenience 메서드를 `maekon-core`에 추가하지 않았는가?
