...
**날짜 (Date)**: 2026-04-18
**범위 (Scope)**: `maekon-core::ports::regime_storage`, `maekon-storage::regime_manager_state_store`, `maekon-analysis::RegimeManager::hydrate_from`, `src-tauri::main::RunEvent::Exit`
...
`maekon-core` 에 새 `RegimeStoragePort` 를, `maekon-storage` 에 `SqliteRegimeManagerStateStore` 를 추가한다. 상태는 **전용(dedicated)** `regime_manager_state` 싱글톤(singleton) 테이블 (v31 마이그레이션) 의 JSON 블롭(blob) 으로 저장한다 — 기존 `regimes` 테이블은 사용하지 않는다.
