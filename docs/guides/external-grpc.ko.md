...
그런 다음 DNS CNAME을 만들거나 Cloudflare의 라우팅 규칙을 사용하여 `maekon.example.com`을 터널로 지정합니다.
...
- `operation` — gRPC 메서드 이름(예: `/maekon.v1.DashboardService/GetSessionStats`).
- `result` — 성공/실패 라벨.
...
3. "포트 사용 중" 오류에 대한 에이전트 로그 확인: `grep -i "port\|address" ~/.maekon/agent.log`.
...
**해결:**
- 인증서 쌍 재생성: `cargo run -p maekon-app --features external-grpc-tools -- generate-external-cert --output-dir ~/.maekon/certs/ --bind-ip 0.0.0.0`
...
3. 에이전트 설정 확인: `grep jwt_expected_ ~/.maekon/config.toml`.
...
외부 gRPC 스트레스 스위트(`crates/maekon-web/tests/external_grpc_stress.rs`)는
