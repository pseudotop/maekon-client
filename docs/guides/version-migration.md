...
|----------|-----------------|----------------|
| macOS | `~/Library/Application Support/maekon/config.json` | `~/Library/Application Support/maekon/data/` |
| Windows | `%APPDATA%\maekon\config.json` | `%LOCALAPPDATA%\maekon\data\` |
| Linux | `~/.config/maekon/config.json` | `~/.local/share/maekon/` |

Source: `crates/maekon-core/src/config_manager.rs`, `config_dir()` and `data_dir()` functions.
...
```bash
# macOS / Linux
cp ~/Library/Application\ Support/maekon/config.json \
   ~/Library/Application\ Support/maekon/config.json.bak

# Windows (PowerShell)
Copy-Item "$env:APPDATA\maekon\config.json" `
          "$env:APPDATA\maekon\config.json.bak"
```
...
Maekon uses SQLite for local storage. The database file is located in the data directory above (filename: `maekon.db` by default, overridable via `storage.db_path` in config).

Schema migrations run automatically at startup via `crates/maekon-storage/src/migration.rs`. The migration runner is idempotent — running it twice on the same database is safe.
...
```bash
sqlite3 ~/Library/Application\ Support/maekon/data/maekon.db \
...
2. Back up `config.json` and `maekon.db`.
...
4. Restore `maekon.db` from backup if the new version ran migrations that are incompatible with the old binary.
...
The `ConsentRecord` schema in `crates/maekon-core/src/consent.rs` was extended with two new fields:
