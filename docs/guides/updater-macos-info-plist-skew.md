...
`/Applications/Maekon.app/Contents/MacOS/maekon` — a file **inside** the `.app` bundle, not the bundle itself.
...
/Applications/Maekon.app/Contents/MacOS/maekon --version

# Info.plist metadata (what Finder shows)
plutil -p /Applications/Maekon.app/Contents/Info.plist \
  | grep -E 'CFBundle(Version|ShortVersionString)'

# Combined diagnostic one-liner
printf "binary: %s\nplist:  %s\n" \
  "$(/Applications/Maekon.app/Contents/MacOS/maekon --version 2>&1 | head -1)" \
  "$(plutil -extract CFBundleShortVersionString raw /Applications/Maekon.app/Contents/Info.plist)"
```

### Verifying code signature integrity

```bash
codesign --verify --verbose=4 /Applications/Maekon.app 2>&1
```
...
```bash
spctl --assess --verbose=4 /Applications/Maekon.app 2>&1
```

Typical post-update output:
```
/Applications/Maekon.app: rejected
...
2. Drag the new `Maekon.app` from the DMG to `/Applications`, replacing the existing one.
...
```bash
#!/bin/bash
# heal-maekon-bundle.sh — re-sync Info.plist and re-register signature
set -euo pipefail

APP=/Applications/Maekon.app
BINARY_VER=$("$APP/Contents/MacOS/maekon" --version | awk '{print $NF}')
...
2. **Atomic swap**: download the new bundle to `~/Library/Application Support/Maekon/pending/Maekon.app.new`, then `ditto --rsrc` the entire tree into place while the current process holds its own binary lock. On Unix this works because files are identified by inode; the running process keeps executing the old binary's inode until next launch.
