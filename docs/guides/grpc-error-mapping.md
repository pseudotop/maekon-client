...
- Status → NetworkError: `crates/maekon-network/src/grpc/error_mapping.rs`
- NetworkError → CoreError: `crates/maekon-network/src/error.rs` `impl From<NetworkError> for CoreError`
- Consumers:
  - `crates/maekon-network/src/grpc/auth_client.rs`
  - `crates/maekon-network/src/grpc/session_client.rs`
  - `crates/maekon-network/src/grpc/context_client.rs`
  - `crates/maekon-network/src/grpc/health_client.rs`
...
Released code strings are immutable (wire contract). Additions require updating `crates/maekon-core/tests/wire_contract_snapshot.expected.txt`.

## Verification

```bash
cargo test -p maekon-network --features grpc error_mapping
