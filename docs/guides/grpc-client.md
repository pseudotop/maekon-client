...
> **Related docs**: [maekon-network crate](../crates/maekon-network.md)
...
cargo build -p maekon-app --features grpc

# Or build maekon-network only
...
```rust
use maekon_network::grpc::{GrpcConfig, UnifiedClient};
use maekon_network::TokenManager;
use std::sync::Arc;
...
```rust
use maekon_network::grpc::GrpcAuthClient;
...
```rust
use maekon_network::grpc::GrpcSessionClient;
...
```rust
use maekon_network::grpc::{GrpcHealthClient, ServingStatus};
...
let auth_status = health.check("maekon.v1.auth.AuthenticationService").await?;
println!("Auth service: {}", auth_status);

// Check all MAEKON service statuses
let statuses = health.check_maekon_services().await;
for s in statuses {
    println!("{}: {}", s.service, s.status);
}
// Example output:
// <server>: SERVING
// maekon.v1.auth.AuthenticationService: SERVING
// maekon.v1.auth.SessionService: SERVING
// maekon.v1.user_context.UserContextService: SERVING
```

### Health Check Before Connection Pattern

```rust
use maekon_network::grpc::{GrpcHealthClient, GrpcConfig, UnifiedClient};
...
```rust
use maekon_network::grpc::UnifiedClient;
...
NO_EMOJI=1 cargo run -p maekon-app --features grpc
```
...
- Proto definitions — `api/proto/maekon/v1/` (see server repository)
