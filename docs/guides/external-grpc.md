...
```bash
cargo run -p maekon-app --features external-grpc-tools -- generate-external-cert \
    --output-dir ~/.maekon/certs/ --bind-ip 0.0.0.0
```
...
jwt_expected_audience = "maekon-agent-{device_id}"
```
...
**macOS (App Firewall):**
```bash
sudo /usr/libexec/ApplicationFirewall/socketfilterfw --add /path/to/maekon-app
sudo /usr/libexec/ApplicationFirewall/socketfilterfw --unblockapp /path/to/maekon-app
```

**Windows (Windows Defender Firewall):**
```powershell
New-NetFirewallRule -DisplayName "MAEKON gRPC" -Direction Inbound `
    -Program "C:\path\to\maekon-app.exe" -Action Allow -Protocol TCP -LocalPort 10092
```
...
Then create a DNS CNAME or use Cloudflare's routing rules to point `maekon.example.com` to the tunnel.
...
- `operation` — gRPC method name (e.g. `/maekon.v1.DashboardService/GetSessionStats`).
...
```bash
sqlite3 ~/.maekon/maekon.db "SELECT * FROM audit_log WHERE timestamp > datetime('now', '-7 days') ORDER BY timestamp DESC LIMIT 100;"
```
...
3. Check agent logs for "port in use" error: `grep -i "port\|address" ~/.maekon/agent.log`.
...
**Fix:**
- Regenerate the cert pair: `cargo run -p maekon-app --features external-grpc-tools -- generate-external-cert --output-dir ~/.maekon/certs/ --bind-ip 0.0.0.0`
...
3. Check the agent config: `grep jwt_expected_ ~/.maekon/config.toml`.
...
- Regenerate the cert immediately:
  ```bash
  cargo run -p maekon-app --features external-grpc-tools -- generate-external-cert \
      --output-dir ~/.maekon/certs/ --bind-ip 0.0.0.0 --force
  ```
...
while true; do
    ...
    grpcurl -H "authorization: Bearer $TOKEN" \
        localhost:10092 list maekon.v1.DashboardService
...
The external gRPC stress suite (`crates/maekon-web/tests/external_grpc_stress.rs`) is gated behind the `stress-test` cargo feature so it never runs in the regular `cargo test --workspace` path. The suite covers three scenarios:
...
```sh
ulimit -n 65536
cargo test -p maekon-web --features stress-test \
