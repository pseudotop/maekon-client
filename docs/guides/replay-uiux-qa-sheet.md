...
- Product surface: `maekon-web` replay flow (`/replay`) plus timeline navigation handoff.
- Primary user jobs:
...
|---|---|---|---|
| Failure UX | Image-load failure has explicit fallback message | Pass | `crates/maekon-web/frontend/src/pages/SessionReplay.tsx` |
| Scrubber | Mouse click/drag seek exists, idle striping and segment colors exist | Pass | `crates/maekon-web/frontend/src/components/TimelineScrubber.tsx` |
| Localization | Replay UI includes Korean fallback literals and fixed `ko-KR` datetime formatting | Partial | `crates/maekon-web/frontend/src/pages/SessionReplay.tsx` |
| Accessibility/Keyboard | Global shortcuts improved, but replay-specific keyboard controls are limited | Partial | `crates/maekon-web/frontend/src/pages/SessionReplay.tsx`, `crates/maekon-web/frontend/src/components/TimelineScrubber.tsx` |
| Mobile input | No touch handlers on scrubber | Fail | `crates/maekon-web/frontend/src/components/TimelineScrubber.tsx` |
| Scale readiness | Event log is full render list (no virtualization/windowing) | Partial | `crates/maekon-web/frontend/src/components/EventLog.tsx` |
