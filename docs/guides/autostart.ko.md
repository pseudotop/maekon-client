...
| 항목 | 값 |
|------|-----|
| 등록 위치 | `~/Library/LaunchAgents/com.maekon.app.plist` |
...
| 항목 | 값 |
|------|-----|
| 1차 메커니즘 | systemd user service `~/.config/systemd/user/maekon.service` (Type=notify) |
| 2차 fallback | XDG Autostart `~/.config/autostart/maekon.desktop` |
| 단일 인스턴스 | D-Bus name `com.maekon.app.SingleInstance` |
...
> **systemd 최소 버전**: systemd user-unit 경로에는 systemd 219 이상이 필요합니다.
> systemd 218 이하 환경에서는 XDG fallback이 자동 적용됩니다.
>
> **이것은 지원 배포판 하한이 아닙니다.** systemd 요구사항은 autostart 가 동작하는지를
> 말할 뿐 릴리스가 설치되는지를 말하지 않습니다. 출하되는 `.deb` 는 `ubuntu-latest`
> 에서 빌드되며, 그 메타데이터 자체가 **Ubuntu 24.04 이상**을 요구합니다 (#11246).
>
> ```
> Depends: libasound2t64 (>= 1.1.0), libc6 (>= 2.39), ..., libglib2.0-0t64
> ```
>
> `libc6 (>= 2.39)` 는 24.04 의 glibc 입니다 — 20.04 는 2.31, 22.04 는 2.35 입니다.
> 그리고 `t64` 패키지명은 64-bit `time_t` 전환에서 온 것으로 24.04 에서 도입됐고,
> 그 이전 릴리스에는 그 이름의 패키지가 없습니다. 의도가 아니라 **산출물에서 나온**
> 서로 독립적인 세 근거입니다.
>
> 더 낮은 배포판이 정책으로 막힌 것은 아닙니다. 아무것도 빌드하거나 검사하지 않기 때문에
> 미지원입니다. 그 환경에서 쓰려면 소스에서 빌드해야 합니다.
...
1. `~/.config/systemd/user/maekon.service` 파일의 SHA-256 hash 계산
...
1. 토글에 마우스를 올려 tooltip 메시지를 확인합니다.
   > Tooltip 텍스트는 `crates/maekon-web/frontend/src/i18n/locales/ko.json` (`settings.autostart.unsupported_*`)에 정의되어 있습니다.
2. 환경별 대처 방법:
...
| `No such file or directory` | 바이너리 경로 변경 | `which maekon` 확인 후 service 파일 `ExecStart=` 수정 |
| 즉시 종료, 로그 없음 | 이중 실행 감지 | 기존 프로세스 확인: `pgrep -a maekon` |

**`TimeoutStartSec` 수정 방법**:

```bash
# ~/.config/systemd/user/maekon.service 편집
...
- **Linux**: `~/.local/share/maekon/logs/` 또는 `journalctl --user -u maekon`
