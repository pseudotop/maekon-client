...
- 전송 DTO SSOT: `crates/maekon-api-contracts`
- 런타임 라우트 SSOT: `crates/maekon-web/src/routes.rs`
- 기계 검증용 인터페이스 스냅샷: `docs/contracts/http-interface-manifest.v1.json`
- 생성된 OpenAPI 스냅샷: `docs/contracts/maekon-web.v1.openapi.yaml`

## 결정 사항

1. 전송 DTO 소유권은 `maekon-api-contracts`에 유지하고, 핸들러는 공개 DTO를 직접 정의하지 않습니다.
2. 라우트 소유권은 `maekon-web/src/routes.rs`에 유지합니다.
...
3. CI OpenAPI 아티팩트 게시:
   - 아티팩트 이름: `maekon-web-v1-openapi`
4. 릴리스 자산 게시:
   - 파일: `maekon-web.v1.openapi.yaml`
