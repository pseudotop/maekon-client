...
// --- Re-exports: models ---
pub use models::{
    ProviderAvailabilityProbeSpec, ProviderCliCompatibilitySpec, ProviderHealthTransportSpec,
    ProviderKnownModelCapabilities, ProviderKnownModelSpec, ProviderLlmCapabilities,
    ProviderModelCapabilityProfile, ProviderModelCapabilityRules,
    ProviderModelCatalogTransportSpec, ProviderModelSupportStatus, ProviderOcrCapabilities,
    ProviderParameterProfile, ProviderParameterSet, ProviderSurfaceCatalog,
    ProviderSurfaceProvisioningSpec, ProviderSurfaceSpec, ProviderSurfaceSupports,
    ProviderTransportSpec, ProviderUnknownModelPolicySet, ProviderVendorProjectionSpec,
...
// ---------------------------------------------------------------------------
// CRITICAL: These items MUST stay in mod.rs (catalog constant + OnceLock cache)
// ---------------------------------------------------------------------------

// Re-uses maekon-core's single compile-time embed of the catalog JSON rather
// than re-embedding the same file with a second `include_str!` (issue #7741).
const PROVIDER_SURFACE_SPECS_JSON: &str = crate::provider_surface::PROVIDER_SURFACE_CATALOG_JSON;
