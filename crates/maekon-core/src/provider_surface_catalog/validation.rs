...
use super::models::{
    ProviderCliCompatibilitySpec, ProviderModelCapabilityProfile, ProviderParameterProfile,
    ProviderSurfaceCatalog, ProviderSurfaceSpec, SubprocessTransportSpec,
};
use super::parsers::{parse_auth_scheme, parse_provider_type, parse_request_shape};
...
pub(super) fn validate_surface_catalog(catalog: &ProviderSurfaceCatalog) -> Result<(), String> {
    ...
    for surface in &catalog.surfaces {
        ...
        if canonical_provider_surface_id(&surface.surface_id).is_none() {
            return Err(format!(
                "Unknown provider surface id '{}' is not registered in maekon-core.",
                surface.surface_id
            ));
        }
        ...
        let catalog_strategy = surface.catalog_strategy;
        ...
        match surface.execution_kind {
            ...
            SurfaceExecutionKind::SubprocessCli => {
                ...
                if subprocess.auth_probe_mode != SubprocessAuthProbeMode::None
...
fn validate_model_capability_profile(
    profile: &ProviderModelCapabilityProfile,
) -> Result<(), String> {
