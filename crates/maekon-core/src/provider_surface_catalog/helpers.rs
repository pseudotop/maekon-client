...
use super::models::{
    ProviderKnownModelSpec, ProviderModelCapabilityProfile, ProviderModelSupportStatus,
    ProviderParameterProfile, ProviderSurfaceSpec,
};
use super::parsers::default_model_support_status;
...
pub(super) fn stability_sort_key(value: SurfaceStability) -> i32 {
    match value {
...
pub(super) fn default_direct_surface_placement_sort_key(value: SurfacePlacementKind) -> i32 {
    match value {
    ...
    catalog
        ...
        .filter(|surface| {
            surface.vendor_id.eq_ignore_ascii_case(vendor_id)
                && surface.execution_kind == SurfaceExecutionKind::DirectHttp
        })
        .max_by(|left, right| {
