...
use crate::models::sync::{ChangeSet, ChangeSetKind, SyncResult};
use crate::ports::change_extractor::ChangeExtractor;
use crate::ports::change_merger::ChangeMerger;
use crate::ports::consent_manager::ConsentManagerPort;
use crate::ports::egress_ledger::EgressLedgerSink;
use crate::ports::erasure_propagation_store::ErasurePropagationStore;
use crate::ports::sync_transport::SyncTransport;
use crate::sync::Hlc;
...
impl SyncHealthState {
    fn as_str(self) -> &'static str {
        match self {
            ...
            Self::Synced => "synced",
            Self::IdleNoExchange => "idle_no_exchange",
            Self::BlockedByConsent => "blocked_by_consent",
            Self::Error => "error",
        }
    }
}
...
pub struct SyncEngine {
    ...
    last_health_state: parking_lot::Mutex<SyncHealthState>,
}
...
impl SyncEngine {
    ...
    pub async fn new(
        ...
        // `local_watermark()` is the DB-wide maximum, so a peer-origin high-watermark
        // row can hide self-origin unsent rows. Until a durable "last successful push"
        // store exists, start from 0 and allow duplicate sends, relying on the
        // receiver's idempotent merge.
        ...
        Self {
            ...
            last_health_state: parking_lot::Mutex::new(SyncHealthState::NeverRun),
        }
    }
    ...
    ///
    /// #7735 E-1: `pub` + `cfg(any(test, feature = "test-support"))` (not the original
    /// `#[cfg(test)] pub(crate)`) because the regression test that calls this lives in
    /// the `maekon-app` crate (`agent_runtime/sync_setup.rs`), not in this crate — a
    /// downstream crate's own `cfg(test)` does not activate `cfg(test)` here, and
    /// `pub(crate)` does not cross the crate boundary. House precedent:
    /// `ports::pii_sanitizer::FakePiiSanitizer`, `ports::ocr_provider::FakeOcrProvider`.
    #[cfg(any(test, feature = "test-support"))]
    pub fn has_egress_sink(&self) -> bool {
    ...
    fn record_push_egress(
        ...
        let consent_state = self
            .consent_manager
            .as_ref()
            .map(|cm| format!("cross_device_sync={}", cm.cross_device_sync_permitted()))
    ...
    pub async fn run_cycle(&self) -> Result<Option<SyncResult>, CoreError> {
        ...
        match self.run_cycle_inner().await {
            Ok((result, health_state)) => {
                if health_state == SyncHealthState::Synced {
                    *self.last_sync_at.lock() = Some(Utc::now());
                }
                *self.last_error.lock() = None;
                *self.last_health_state.lock() = health_state;
                Ok(result)
            }
            Err(e) => {
                *self.last_error.lock() = Some(e.to_string());
                *self.last_health_state.lock() = SyncHealthState::Error;
                Err(e)
            }
        }
    }
    ...
    async fn run_cycle_inner(&self) -> Result<(Option<SyncResult>, SyncHealthState), CoreError> {
        ...
        if let Some(health_state) = self.propagate_pending_erasure_status().await? {
            return Ok((None, health_state)); // a deletion cycle does no normal data sync
        }
        ...
        // #7728: this `if let Some` — skip the consent check entirely when NO
        // ConsentManager is installed — is deliberately NOT routed through
        // `ConsentGate` (which would fail-closed on a missing manager and
        // change this documented behavior; see `SyncEngine::new`'s doc
        // comment: "When `None`, consent checks are skipped (sync always
        // runs)"). In production the composition root always injects a real
        // `Arc<ConsentManager>` here (`capture_services.rs`), so this branch
        // is a defensive fallback, not a reachable fail-open — unlike the
        // `scheduler/config.rs` telemetry site this issue fixes. The boolean
        // READ itself is still routed through the shared
        // `cross_device_sync_permitted()` primitive so it cannot diverge from
        // the OTHER cross_device_sync read at `record_egress_ledger` above.
        if let Some(ref cm) = self.consent_manager {
            if !cm.cross_device_sync_permitted() {
                debug!("sync skipped: cross_device_sync consent not granted");
                return Ok((None, SyncHealthState::BlockedByConsent));
            }
        }
        ...
        let health_state = if exchanged {
            SyncHealthState::Synced
        } else {
            SyncHealthState::IdleNoExchange
        };
        Ok((merge_result, health_state))
    }
    ...
    pub async fn propagate_pending_erasure(&self) -> Result<bool, CoreError> {
        Ok(self.propagate_pending_erasure_status().await?.is_some())
    }

    async fn propagate_pending_erasure_status(&self) -> Result<Option<SyncHealthState>, CoreError> {
        if let Some((erasure_id, persist)) = self.deletion_to_propagate() {
            ...
            let delivered = self.push_deletion_event(&erasure_id, persist).await?;
            return Ok(Some(if delivered {
                SyncHealthState::Synced
            } else {
                SyncHealthState::IdleNoExchange
            }));
    ...
    async fn push_deletion_event(&self, id: &str, persist: bool) -> Result<bool, CoreError> {
    ...
    ///
    /// #7725: this deliberately calls the DETERMINISTIC (non-jittered)
    /// `maekon_core::backoff::exponential_delay` rather than
    /// `maekon_network::resilience::jittered_backoff_delay`. Two reasons:
    /// (1) `undeliverable_erasure_backs_off_then_retries` (below) advances a
    /// paused clock by exactly 31s to cross the 30s first-failure window —
    /// injecting jitter (up to +25%) would make that assertion flaky; (2)
    /// `sync_engine` is compiled unconditionally (`mod sync_engine;` in
    /// `main.rs` carries no feature gate), while `maekon-network` is an
    /// optional dependency gated behind the `analysis`/`server` features, so a
    /// `maekon-network` import here would break `--no-default-features`
    /// builds. `maekon-core` is always available.
    fn bump_erasure_backoff(&self, id: &str) {
        ...
        let delay = crate::backoff::exponential_delay(
            st.failures - 1,
            std::time::Duration::from_secs(30),
            std::time::Duration::from_secs(1800),
        );
        st.retry_after = Some(tokio::time::Instant::now() + delay);
    }
    ...
    pub fn health_state(&self) -> &'static str {
        self.last_health_state.lock().as_str()
    }

    /// Discover known peers via the configured transport.
    pub async fn discover_peers(&self) -> Result<Vec<crate::models::sync::PeerInfo>, CoreError> {
...
mod tests {
    ...
    use crate::models::sync::PeerInfo;
    use crate::ports::consent_manager::ConsentManagerPort;
    ...
    #[test]
    fn transport_retention_doc_comment_stays_separate_from_erasure_propagation() {
        let source = include_str!("sync_engine.rs");
        let marker = "    pub async fn enforce_transport_retention";
        let before_fn = &source[..source.find(marker).expect("retention function exists")];
        let block_start = before_fn
            .rfind("\n\n")
            .expect("retention function has a doc comment");
        let retention_doc = &before_fn[block_start..];

        assert!(retention_doc.contains("transport's `enforce_retention`"));
        assert!(!retention_doc.contains("GDPR Art. 17 erasure"));
    }
    ...
    struct OverlapDetectingTransport {
        ...
        max_active_pulls: AtomicUsize,
    }

    impl OverlapDetectingTransport {
        fn max_active_pulls(&self) -> usize {
            self.max_active_pulls.load(Ordering::SeqCst)
        }
    }

    #[async_trait]
    impl SyncTransport for OverlapDetectingTransport {
        async fn push(&self, _changes: &ChangeSet) -> Result<usize, CoreError> {
            Ok(0)
        }

        async fn pull(&self, _since: &Hlc) -> Result<Option<ChangeSet>, CoreError> {
            let active = self.active_pulls.fetch_add(1, Ordering::SeqCst) + 1;
            self.max_active_pulls.fetch_max(active, Ordering::SeqCst);
            tokio::time::sleep(std::time::Duration::from_millis(50)).await;
            self.active_pulls.fetch_sub(1, Ordering::SeqCst);
            Ok(None)
        }
    ...
    #[tokio::test]
    async fn health_state_marks_consent_blocked_cycle() {
        let engine = SyncEngine::new(
            Arc::new(MockExtractor::new(ChangeSet::default())),
            Arc::new(MockMerger {
                apply_count: AtomicUsize::new(0),
        ...
        let (last_sync_at, last_error) = engine.health_status();
        assert_eq!(engine.health_state(), "blocked_by_consent");
        assert!(last_sync_at.is_none());
        assert!(last_error.is_none());
    }

    #[tokio::test]
    async fn health_state_marks_idle_cycle_without_exchange() {
        let engine = SyncEngine::new(
            Arc::new(MockExtractor::new(ChangeSet::default())),
            Arc::new(MockMerger {
                apply_count: AtomicUsize::new(0),
        ...
        let (last_sync_at, last_error) = engine.health_status();
        assert_eq!(engine.health_state(), "idle_no_exchange");
        assert!(last_sync_at.is_none());
        assert!(last_error.is_none());
    }

    #[tokio::test]
    async fn run_cycle_serializes_overlapping_calls() {
        let transport = Arc::new(OverlapDetectingTransport::default());
        let engine = Arc::new(
            SyncEngine::new(
                Arc::new(MockExtractor::new(ChangeSet::default())),
                Arc::new(MockMerger {
        ...
        assert_eq!(transport.max_active_pulls(), 1);
    }
