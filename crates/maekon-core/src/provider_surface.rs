...
fn provider_surface_specs() -> Option<&'static [ProviderSurfaceProjection]> {
    match KNOWN_PROVIDER_SURFACES.get_or_init(load_provider_surface_specs) {
        ...
        Err(error) => {
            tracing::warn!(
                error = %error,
                "Failed to load provider surface catalog inside maekon-core."
            );
            None
        }
    }
...
fn provider_vendor_projections() -> Option<&'static [ProviderVendorProjection]> {
    match KNOWN_PROVIDER_VENDOR_PROJECTIONS.get_or_init(load_provider_vendor_projections) {
        ...
        Err(error) => {
            tracing::warn!(
                error = %error,
                "Failed to load provider vendor projection metadata inside maekon-core."
            );
            None
        }
    }
...
fn provider_vendors() -> Option<&'static [ProviderVendorInfo]> {
    match KNOWN_PROVIDER_VENDORS.get_or_init(load_provider_vendors) {
        ...
        Err(error) => {
            tracing::warn!(
                error = %error,
                "Failed to load provider vendor metadata inside maekon-core."
            );
            None
        }
    }
