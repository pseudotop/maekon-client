...
//! The process-global battery-saver static and the 3-arg wrapper functions
//! that read it stay behind in `maekon_app::scheduler::schedule` (they need a
//! process-wide flag, which is a composition-root concern); this module hosts
//! only the pure, explicit-arg policy functions those wrappers delegate to.
//!
//! This module exposes several pure functions consumed by the scheduler:
//!
...
//!
//! * [`decide_monitor_tick`] / [`should_release_idle_frame`] — power-aware
//!   monitor-loop cadence decisions (idle backoff + battery-saver throttle).

use crate::config::{AppConfig, Weekday};
use crate::consent::ConsentPermissions;
...
///
/// `NotificationManager` (in the `maekon-app` scheduler crate) implements
/// this trait, so it is used directly in production. Tests use the
/// [`RecordingNotifier`] mock implementation.
#[async_trait]
pub trait TsNotifier: Send + Sync {
...
// ── weekday_pred / should_run_now_with_time (moved from scheduler/schedule.rs) ──
...
/// Production call-sites should use `should_run_now` (in the `maekon-app`
/// scheduler crate) which calls `chrono::Local::now()` internally.
///
...
pub fn should_run_now_with_time(config: &AppConfig, now: DateTime<Local>) -> bool {
...
pub fn tracking_schedule_active(cfg: &AppConfig, now: DateTime<Local>) -> bool {
...
pub fn capture_permitted_now_with_power(
    cfg: &AppConfig,
    consent: &ConsentPermissions,
    capture_paused: bool,
...
pub fn audio_capture_permitted_now_with_power(
    cfg: &AppConfig,
    consent: &ConsentPermissions,
    capture_paused: bool,
    battery_saver_active: bool,
...
pub(crate) async fn evaluate_and_notify_transitions<N: TsNotifier + ?Sized>(
    cfg: &AppConfig,
    prev_active: bool,
    now_active: bool,
...
pub async fn tick_ts_notifications(
    ...
    // #7735 E-3: was `Option<&crate::notification_manager::NotificationManager>`
    // (a concrete src-tauri type core cannot see) — now the narrow trait-object
    // port; `impl TsNotifier for NotificationManager` lives in src-tauri.
    notifier: Option<&dyn TsNotifier>,
    prev_ts_active: &mut bool,
    last_ts_notified_at: &mut Option<std::time::Instant>,
) {
    let default_cfg;
    let cfg: &AppConfig = match config {
        Some(cfg) => cfg,
        None => {
...
/// threshold.
const MONITOR_IDLE_BACKOFF_SECS: u64 = 30;
...
pub fn should_release_idle_frame(idle_secs: u64) -> bool {
...
const MONITOR_IDLE_CADENCE_SECS: u64 = 5;
...
const MONITOR_BATTERY_CADENCE_SECS: u64 = 3;
...
pub struct MonitorTickDecision {
    pub process: bool,
...
// NOTE (#7735 E-3): `impl TsNotifier for NotificationManager` no longer lives
...
mod tests {
    use super::*;
    use crate::config::{TrackingScheduleConfig, TrackingWindow};
