...
impl TokenManager {
    ...
    /// `invalidate_all_user_sessions(except_current_session_id=None)` 경유 모든
    /// 디바이스의 토큰/세션 폐기. UI 의 "Sign out of all devices" 옵션 연결점.
    ///
    /// Local state 도 같이 clear (`logout` 과 동일 패턴) — 본 디바이스 포함 모든
    /// 디바이스 logout. 서버측 실패 시에도 local state 는 clear (UX 일관성).
    ///
    /// # Errors
    ///
    /// Token 만료 또는 부재 시 `CoreError::Auth`. 서버 호출 실패는 warn 로 기록 후 무시
    /// (local state cleared — `logout` 과 동일 정책).
...
mod tests {
    ...
    // OOS-N15 (2026-05-05): logout + logout_all_sessions 테스트
    #[tokio::test]
    ...
    async fn logout_all_sessions_clears_state_even_when_server_fails() {
        ...
        let login_mock = server
            ...
            .create_async()
            .await;
        // server returns 500 — local state still cleared (UX 일관성, logout 패턴 정합)
        let logout_all_mock = server
            .mock("DELETE", "/api/v1/auth/tokens/all")
            .match_header("authorization", "Bearer jwt_abc")
            .with_status(500)
