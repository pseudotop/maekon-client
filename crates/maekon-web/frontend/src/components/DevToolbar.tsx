...
const STANDALONE_KEY = 'maekon-web-standalone-mode'
...
export function DevToolbar() {
  ...
  const apiBase = window.__MAEKON_WEB_PORT__
    ? `http://127.0.0.1:${window.__MAEKON_WEB_PORT__}`
  ...
  return (
    ...
    <div className="fixed bottom-12 right-4 z-[9999] bg-gray-900 text-white text-xs rounded-lg shadow-2xl p-3 w-72 border border-yellow-500/50">
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between">
          <span>Port</span>
          <span className="text-gray-400">{window.__MAEKON_WEB_PORT__ ?? 'default'}</span>
        </div>
