...
// patterns for clarity over performance. (Enforced workspace-wide via
...
pub use required_deps::WebServerRequiredDeps;
...
impl WebServer {
    /// `event_tx` is required (#7738 D-1): the composition root's shared event
    /// bus is passed directly instead of `new` minting a throwaway channel
    /// that `with_runtime_bindings` immediately overwrote. Tests that don't
    /// care about the broadcast channel create their own with
    /// `broadcast::channel(N)` and pass the sender straight in.
    pub fn new(
    ...
    /// Consume the VERIFIED-unconditional dependency set (#7738 D-2). Every
    /// field is destructured by name (no `..`), so a field added to
    /// `WebServerRequiredDeps` without a matching line here fails to compile
    /// instead of silently staying unset.
    pub fn with_required_deps(mut self, deps: WebServerRequiredDeps) -> Self {
        ...
        self.state.auth.local_auth_token = Some(local_auth_token);
        self.state.core.frames_dir = Some(frames_dir);
        self.state.diagnostics.pii_sanitizer = Some(pii_sanitizer);
        self.state.diagnostics.runtime_log_provider = Some(runtime_log_provider);
        self.state.diagnostics.system_info_provider = Some(system_info_provider);
        self.state.diagnostics.provider_cli_diagnostics = Some(provider_cli_diagnostics);
        self
    }
    ...
    /// No `WebServerRuntimeBindings` coverage exists for these two fields —
    /// they live in a different crate-feature namespace (maekon-web's own
    /// `grpc-dashboard-external`, not src-tauri's `server`/`analysis`) — so
    /// they keep individual setters (#7738 D-3).
    ...
    /// Consume the sparse OPTIONAL/CONDITIONAL/cfg dependency bundle (#7738
    /// D-3) — see [`WebServerRuntimeBindings`] for which fields stay `Option`
    /// and why (automation-disabled, capture-degraded, etc.). Applied only
    /// when `Some`.
    ...
    pub fn build_router(mut state: AppState) -> Router {
        ...
        let allowed_origins: Vec<HeaderValue> = (maekon_core::config::DEFAULT_WEB_PORT
            ...
            .chain(std::iter::once(
                "tauri://localhost"
                    .parse()
                    .unwrap_or_else(|error| panic!("static URL: {error}")),
            ))
            .chain(std::iter::once(
...
pub(crate) mod test_local_auth {
    ...
    /// Default broadcast-channel capacity for `test_app_state()`. Matches what
    /// every pre-consolidation handler test copy used, EXCEPT `stream.rs`'s SSE
    /// tests, which need a wider buffer — those call
    /// `test_app_state_with_event_capacity(128)` directly instead.
    const DEFAULT_TEST_EVENT_CAPACITY: usize = 16;

    /// Canonical in-memory `AppState` builder for handler unit tests (#7729
    /// ctd-W2 G2). Replaces ~11 near-identical hand-rolled copies of the same
    /// three lines (in-memory SQLite storage retention=30 + broadcast channel +
    /// `AppState::with_core`) that were previously duplicated across
    /// `handlers/*.rs` test modules.
    pub(crate) fn test_app_state() -> AppState {
        test_app_state_with_event_capacity(DEFAULT_TEST_EVENT_CAPACITY)
    }

    /// Same as [`test_app_state`], but with a caller-chosen broadcast-channel
    /// capacity (`stream.rs`'s SSE tests need 128 instead of the default 16 to
    /// avoid dropping events under test load).
    pub(crate) fn test_app_state_with_event_capacity(capacity: usize) -> AppState {
        let storage = Arc::new(SqliteStorage::open_in_memory(30).expect("in-memory sqlite"));
        let (event_tx, _) = broadcast::channel(capacity);
        AppState::with_core(storage, event_tx)
    }

    /// Same as [`test_app_state`], but also returns the underlying
    /// `SqliteStorage` handle, for tests that seed data directly through the
    /// storage port (bypassing the HTTP surface under test).
    pub(crate) fn test_app_state_with_storage() -> (AppState, Arc<SqliteStorage>) {
        let storage = Arc::new(SqliteStorage::open_in_memory(30).expect("in-memory sqlite"));
        let (event_tx, _) = broadcast::channel(DEFAULT_TEST_EVENT_CAPACITY);
    ...
    /// Convenience combinator for the common no-custom-state case: a fresh
    /// canonical [`test_app_state`] wrapped in [`authed_loopback_router`].
    pub(crate) fn loopback_app() -> axum::Router {
        authed_loopback_router(test_app_state())
    }
}
...
mod tests {
    ...
    fn web_server_url() {
        ...
        let server = WebServer::new(storage, event_tx, WebConfig::default());
    ...
    async fn cors_allows_tauri_localhost_origin_for_embedded_webview() {
        ...
        let state = crate::test_local_auth::test_app_state_with_event_capacity(8);
    ...
    /// reusable dev-dependency impl reachable from this crate (their only
    /// implementers live in `src-tauri`/`maekon-monitor`, which `maekon-web`
    /// must not depend on — ADR-001 §7), so they get manual stub mocks
    /// (project convention: no mockall).
    fn full_required_deps(storage: &Arc<SqliteStorage>) -> WebServerRequiredDeps {
        use crate::services::provider_cli_diagnostics::ProviderCliDiagnosticsProvider;
        use maekon_api_contracts::update::UpdateStatus;
        use maekon_automation::audit::{AuditLogAdapter, AuditLogger};
        use maekon_core::error::CoreError;
        use maekon_core::models::bug_report::RuntimeLogSnapshot;
        use maekon_core::models::system::StaticSystemInfo;
        use maekon_core::ports::pii_sanitizer::FakePiiSanitizer;
        use maekon_core::ports::provider_cli_diagnostics::ProviderCliDiagnosticsFuture;
        ...
        impl RuntimeLogProvider for StubRuntimeLogProvider {
            async fn snapshot(&self, _line_limit: usize) -> Result<RuntimeLogSnapshot, CoreError> {
        ...
        impl ProviderCliDiagnosticsProvider for StubProviderCliDiagnostics {
        ...
        // (mirrors update_control.rs's own tests: `_action_rx` dropped, unused).
        let (action_tx, _action_rx) = tokio::sync::mpsc::unbounded_channel();
        let update_control =
            crate::update_control::UpdateControl::new(action_tx, UpdateStatus::default());
        let audit_logger_inner = Arc::new(tokio::sync::RwLock::new(AuditLogger::default()));
        ...
        WebServerRequiredDeps {
            ...
            audit_logger: Arc::new(AuditLogAdapter::new(audit_logger_inner)),
            config_manager,
            update_control,
            local_auth_token: Arc::from("test-required-deps-token"),
            frames_dir: std::path::PathBuf::from("/tmp/maekon-web-required-deps-test"),
            pii_sanitizer: Arc::new(FakePiiSanitizer::new()),
            runtime_log_provider: Arc::new(StubRuntimeLogProvider),
            system_info_provider: Arc::new(StubSystemInfoProvider),
    ...
    fn memory_graph_binding_is_applied_to_core_state() {
        // #7738 D-2: memory_graph is now a REQUIRED (always-Some) field — this
        // test now proves `with_required_deps`'s destructuring-bind transfers
        // the SAME Arc end to end (Port Instance Sharing), not just that the
        // field gets populated (the type system now guarantees that part).
        let storage = Arc::new(SqliteStorage::open_in_memory(30).unwrap());
        let (event_tx, _) = broadcast::channel(8);
        let deps = full_required_deps(&storage);
        let expected_memory_graph = deps.memory_graph.clone();
        let server =
            WebServer::new(storage, event_tx, WebConfig::default()).with_required_deps(deps);
        let applied_memory_graph = server
            .state
            .core
            .memory_graph
            .expect("memory_graph is required — always Some by construction");
        assert!(
    ...
    fn web_server_runtime_bindings_apply_scalar_runtime_state() {
        ...
        // #7738 D-2: frames_dir moved from WebServerRuntimeBindings to
        // WebServerRequiredDeps — set it on the required-deps literal instead.
        let frames_dir = std::path::PathBuf::from("/tmp/maekon-web-runtime-bindings");
        let mut deps = full_required_deps(&storage);
        deps.frames_dir = frames_dir.clone();

        let server = WebServer::new(storage, event_tx, WebConfig::default())
            .with_required_deps(deps)
            .with_runtime_bindings(WebServerRuntimeBindings {
    ...
    fn test_state_with_config_manager(config_manager: Option<ConfigManager>) -> AppState {
        // #7738 D-4: funnel through the canonical test-state helper.
        let mut state = crate::test_local_auth::test_app_state_with_event_capacity(128);
    ...
    async fn local_auth_fails_closed_when_no_token_provisioned() {
        ...
        // #7738 D-4: funnel through the canonical test-state helper.
        let state = crate::test_local_auth::test_app_state_with_event_capacity(8); // local_auth_token = None
