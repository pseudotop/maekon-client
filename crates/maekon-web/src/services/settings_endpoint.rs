...
fn validate_endpoint_model_compatibility(
            ...
            .map_err(ApiError::BadRequest)?
    {
        ...
        .map_err(ApiError::BadRequest)?
        {
            maekon_api_contracts::ai_providers::ProviderModelSupportStatus::Unsupported => {
                return Err(ApiError::BadRequest(format!(
                    "Model '{model}' is not marked as supporting structured JSON output required by the selected OCR surface."
                )));
            }
            maekon_api_contracts::ai_providers::ProviderModelSupportStatus::Unknown => {
                warn!(
                    provider = ?provider_type,
                    surface_id = ?surface_id,
                    model = %model,
            ...
            maekon_api_contracts::ai_providers::ProviderModelSupportStatus::Supported => {}
        }
    }
...
pub(crate) fn surface_uses_no_auth(surface_id: &str) -> bool {
    maekon_api_contracts::provider_specs::provider_surface_spec(surface_id)
        .map(|surface| {
            surface
                .llm_transport
                .as_ref()
