use maekon_api_contracts::ai_providers::{ProviderDiscoveredModel, ProviderModelSupportStatus};
...
pub(crate) fn build_model_details(
    ...
    models
        .iter()
        .map(|model| {
            let known = maekon_api_contracts::provider_specs::known_model_spec_for_surface(
                surface_id, &model.id,
            )
