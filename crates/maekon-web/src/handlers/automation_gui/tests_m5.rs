...
use maekon_api_contracts::automation_gui::{
...
use maekon_core::models::intent::{ElementBounds, IntentConfig};
...
impl OverlayDriver for M5MockOverlayDriver {
    ...
    async fn show_detection(
        &self,
        _scene: &maekon_core::models::ui_scene::UiScene,
    ) -> Result<(), maekon_core::error::CoreError> {
        Ok(())
    }

    async fn clear_detection(&self) -> Result<(), maekon_core::error::CoreError> {
        Ok(())
    }
}
...
fn make_controller_with(
    ...
    let sandbox: Arc<dyn maekon_core::ports::sandbox::Sandbox> = Arc::new(NoOpSandbox);
