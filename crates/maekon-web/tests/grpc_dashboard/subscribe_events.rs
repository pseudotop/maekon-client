// SubscribeEvents streaming integration tests — event-type filtering, rate
// limiting, DropAccumulator emission, ServerLoadHint under high CPU,
// concurrent clients, AiRuntimeStatus snapshot + PII sanitisation,
// FailingStorage-driven idle invariants, and an HTTP/2 keepalive source
// check. Split from `grpc_dashboard_integration.rs` by scenario family
// (#7730). `crate::failing_storage` (the `FailingStorage` test double) is
// declared at the crate root — see `tests/grpc_dashboard_integration.rs` —
// so its `#[path]` stays resolvable regardless of this module's nesting.

use std::sync::Arc;
use std::time::Duration;

use crate::common::{
    connect_dashboard_client, in_memory_storage, pick_free_port, wait_for_server_ready,
    PassthroughPiiSanitizer, TEST_LOCAL_AUTH_TOKEN,
};

use maekon_api_contracts::stream::{AiRuntimeStatus, FrameUpdate, IdleUpdate, RealtimeEvent};
...
fn explicit_cfg(
    port: u16,
    storage: Arc<dyn maekon_web::storage_port::WebStorage>,
...
async fn subscribe_events_drops_when_rate_limited() {
    use maekon_web::grpc::BURST_CAPACITY;
    ...
    let mut frame_count = 0u32;
    let mut dropped_signal: Option<maekon_web::proto::dashboard::v1::DroppedEventsSignal> = None;
...
async fn subscribe_events_emits_channel_lag_drop_signal() {
    ...
    let mut channel_lag_signal: Option<maekon_web::proto::dashboard::v1::DroppedEventsSignal> =
        None;
...
async fn subscribe_events_emits_server_load_hint_on_high_cpu() {
    use maekon_web::proto::dashboard::v1::server_load_hint::Level as HintLevel;
...
async fn subscribe_events_emits_idle_even_when_storage_fails() {
    ...
    let port = pick_free_port();
    let sqlite = maekon_storage::sqlite::SqliteStorage::open_in_memory(30).expect("sqlite");
    let failing = Arc::new(FailingStorage::new(Arc::new(sqlite)).with_fail_start_idle());
    let storage: Arc<dyn maekon_web::storage_port::WebStorage> = failing;
