...
use maekon_core::ports::secret_store::{secret_env_var_name, SecretStore};
...
impl EnvSecretStore {
    ...
    fn read_only_error() -> CoreError {
        CoreError::SecretStoreError { code: maekon_core::error_codes::SecretCode::Failed, message: "environment-backed secret store is read-only; modify the environment source instead"
                .to_string(), }
    }
}
