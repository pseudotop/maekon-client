        ...
        async fn extract_window_elements(
            ...
            let conn: AccessibilityConnection =
                AccessibilityConnection::new().await.map_err(|e| {
                    Self::record_failure();
                    CoreError::PermissionDenied { code: maekon_core::error_codes::PermissionCode::PermissionDenied, message: format!("AT-SPI2 D-Bus connection failed. Ensure at-spi2-core is installed: {e}") }
                })?;
