use maekon_automation::audit::{AuditLogger, AuditStatus};
...
fn perf_budget_audit_insert_and_query() {
    ...
    let insert_budget_ms = perf_budget_ms("MAEKON_PERF_BUDGET_AUDIT_INSERT_MS", 1_800);
    let query_budget_ms = perf_budget_ms("MAEKON_PERF_BUDGET_AUDIT_QUERY_MS", 250);
