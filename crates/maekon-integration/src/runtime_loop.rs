...
use maekon_http_core::resilience::{
    scale_duration, RetryBackoffGate, RetryBackoffPolicy, RetryHint,
};

/// Answer the one question `RetryBackoffGate::on_failure` asks: did the server
/// hand us a `Retry-After` hint?
///
/// ADR-034: this used to be `core_to_network_error`, a full `CoreError` →
/// `NetworkError` mapping whose only consumer was the gate — which then matched
/// on `RateLimited` and threw the rest of the conversion away. The gate now
/// takes [`RetryHint`] directly (so `maekon-http-core` stays free of any
/// adapter's error type), and this shrinks to the part that was ever used.
fn retry_hint(e: &CoreError) -> RetryHint {
    match e {
        CoreError::RateLimit {
            ...
            tokio::select! {
                _ = connect_interval.tick() => {
                    ...
                    if let Err(error) = self.run_connect_cycle().await {
                        let delay = connect_gate.on_failure(now, retry_hint(&error));
                ...
                _ = heartbeat_interval.tick() => {
                    ...
                    match self.run_heartbeat_cycle().await {
                        ...
                        Err(error) => {
                            let delay = heartbeat_gate.on_failure(now, retry_hint(&error));
                ...
                _ = egress_interval.tick() => {
                    ...
                    match self.run_egress_cycle().await {
                        ...
                        Err(error) => {
                            let delay = egress_gate.on_failure(now, retry_hint(&error));
                ...
                _ = inbox_interval.tick() => {
                    ...
                    match self.run_inbox_cycle().await {
                        ...
                        Err(error) => {
                            let delay = inbox_gate.on_failure(now, retry_hint(&error));
...
mod tests {
    ...
    use super::*;
    use crate::test_support::FakeIntegrationSessionPort;
    use maekon_core::models::integration::{
