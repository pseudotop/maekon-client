...
use maekon_http_core::provider_error_body::provider_error_body_state;
...
pub(super) fn map_integration_body_error(
    e: maekon_http_core::outbound::BodyReadError,
) -> CoreError {
    let message = match e {
...
pub(crate) fn reject_cleartext_remote_url(url: &str, field: &str) -> Result<(), CoreError> {
    ...
    let is_cleartext = reqwest::Url::parse(url)
        ...
        .unwrap_or(false);
    if is_cleartext && !maekon_http_core::outbound::host_is_loopback(url) {
