...
use crate::transport::{IntegrationEgressTransportClient, IntegrationEgressTransportResponse};
use crate::{
...
use maekon_core::models::integration::{IntegrationAckCursor, QueuedIntegrationEgressMessage};
...
impl IntegrationEgressTransportClient for HttpsIntegrationEgressTransportClient {
    async fn send_messages(
        ...
        let body = maekon_http_core::outbound::read_body_capped(
            response,
            maekon_http_core::outbound::MAX_INTEGRATION_RESPONSE_BYTES,
        )
        .await
