...
use crate::IntegrationRequestProof;
use async_trait::async_trait;
use chrono::Utc;
use futures::{SinkExt, StreamExt};
use maekon_api_contracts::integration::{
...
impl IntegrationAuthPort for StaticAuthPort {
    ...
    async fn current_auth_status(
        &self,
    ) -> Result<maekon_core::models::integration::IntegrationAuthStatus, CoreError> {
        Ok(maekon_core::models::integration::IntegrationAuthStatus {
            profile_kind: maekon_core::models::integration::IntegrationAuthProfileKind::EnvToken,
            status: maekon_core::models::integration::IntegrationAuthStatusKind::Ready,
            interactive: false,
            authenticated: true,
    ...
    async fn start_device_authorization(
        ...
        _requested_scopes: &[IntegrationCapabilityScope],
        _resource_indicator: Option<&str>,
    ) -> Result<maekon_core::models::integration::IntegrationDeviceAuthorizationFlow, CoreError>
    {
                    ...
                    match message.unwrap() {
                        Message::Text(text) => {
                            ...
                            if auto_ack_insights {
                                if let Ok(event) = serde_json::from_str::<crate::cloudevents::IntegrationCloudEvent<InsightPacket>>(&text) {
...
async fn egress_transport_uses_websocket_channel_with_queue_id_extension() {
    ...
    let live_messages = live_messages.lock().unwrap().clone();
    let event: crate::cloudevents::IntegrationCloudEvent<InsightPacket> =
        serde_json::from_str(&live_messages[0]).unwrap();
            ...
            .send_messages(
                ...
                vec![QueuedIntegrationEgressMessage {
                    ...
                    payload: IntegrationOutboundPayload::PromptReceipt(
                        maekon_core::models::integration::IntegrationPromptReceipt {
                            receipt_id: "receipt-011".to_string(),
                            prompt_id: "prompt-011".to_string(),
                            action:
                                maekon_core::models::integration::IntegrationPromptReceiptAction::Acknowledged,
                            occurred_at: Utc::now(),
                            reason: None,
                        },
                    ),
