...
> **将零散的桌面活动，转化为每日聚焦成果。**
> Maekon 将本地工作信号整理为专注时间线、下一步行动候选和策略门控自动化路径。
...
- 软限制（`MAEKON_TARGET_SOFT_LIMIT_MB`，默认 `8192`）：清理 `target/debug/incremental`，若仍超出则清理 `target/debug/deps`
- 硬限制（`MAEKON_TARGET_HARD_LIMIT_MB`，默认 `12288`）：额外清理 `target/debug/build`
- 自动清理开关：`MAEKON_TARGET_AUTO_PRUNE=1`（默认）/ `0`（禁用）
...
Maekon 是应用显示名称。当前发布文件名会有意保留 `maekon-*`
格式，以保持安装器、更新器和校验和兼容性。

| 平台 | 文件 |
|--------|------|
| macOS Universal (DMG 安装包) | `maekon-macos-universal.dmg` |
| macOS Universal (PKG 安装包) | `maekon-macos-universal.pkg` |
...
兼容性说明：`MAEKON_*` 环境变量、`maekon` CLI 命令、
`com.maekon.app` 以及现有 config/data 路径在此发布线中仍作为稳定的
技术标识符保留。
...
基于 Hexagonal Architecture（Ports & Adapters）的 15 包 Cargo workspace。14 个 crate 位于 `crates/` 下，主二进制/composition root 位于 `src-tauri/`（Tauri v2，包名 `maekon-app`）。
...
├── crates/
│   ├── maekon-core/       # 领域模型 + Port trait + 错误定义 + 配置
│   ├── maekon-network/    # HTTP/SSE/WebSocket/gRPC、压缩、认证
│   ├── maekon-suggestion/ # 建议接收与处理
│   ├── maekon-storage/    # SQLite 本地存储 + schema migration
│   ├── maekon-monitor/    # 系统指标、活动窗口、活动追踪
│   ├── maekon-vision/     # 屏幕捕获、增量编码、OCR、PII 过滤
│   ├── maekon-web/        # 本地 Web 仪表盘（Axum REST + React）
│   ├── maekon-automation/ # 自动化控制、策略、审计日志
│   ├── maekon-analysis/   # LLM 分析流水线、regime 分类
│   ├── maekon-embedding/  # 向量 embedding + INT8 量化
│   ├── maekon-audio/      # 音频捕获 + STT 流水线
│   ├── maekon-sandbox-worker/ # out-of-process 沙箱执行器
│   ├── maekon-api-contracts/ # 共享 API 类型契约
│   └── maekon-lint/       # workspace lint 工具
└── docs/
    ├── crates/             # 各 crate 详细文档
...
| Crate | 职责 | 文档 |
|----------|------|------|
| maekon-core | 领域模型、Port 接口 | [详情](./docs/crates/maekon-core.md) |
| maekon-network | HTTP/SSE/WebSocket/gRPC、压缩、认证 | [详情](./docs/crates/maekon-network.md) |
| maekon-vision | 捕获、增量编码、OCR | [详情](./docs/crates/maekon-vision.md) |
| maekon-monitor | 系统指标、活动窗口 | [详情](./docs/crates/maekon-monitor.md) |
| maekon-storage | SQLite、离线存储 | [详情](./docs/crates/maekon-storage.md) |
| maekon-suggestion | 建议队列、反馈 | [详情](./docs/crates/maekon-suggestion.md) |
| maekon-web | 本地 Web 仪表盘、REST API | [详情](./docs/crates/maekon-web.md) |
| maekon-automation | 自动化控制、审计日志 | [详情](./docs/crates/maekon-automation.md) |
| maekon-analysis | LLM 分析流水线、regime 分类 | — |
| maekon-embedding | 向量 embedding、INT8 量化 | — |
| maekon-audio | 音频捕获、STT 流水线 | — |
| maekon-sandbox-worker | 沙箱自动化动作执行器 | — |
| maekon-api-contracts | 共享 API 类型契约 | — |
| maekon-lint | workspace lint 工具（language-check） | — |
...
1. 在 `maekon-core` 中定义 Port trait
