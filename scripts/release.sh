...
# ── 변경사항 커밋 ─────────────────────────────────────────────────────────────
info "변경사항을 커밋합니다..."
git add "${CHANGELOG}" Cargo.toml Cargo.lock crates/maekon-web/frontend/package.json
...
echo "     MAEKON_RELEASE_DECISION_MANIFEST=<manifest.json> ./scripts/publish-rc-tag.sh ${VERSION}"
echo ""
