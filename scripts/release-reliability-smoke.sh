...
ROOT_DIR="${MAEKON_SMOKE_REPO_ROOT:-$(cd "$SCRIPT_DIR/.." && pwd)}"
CARGO_CMD="$ROOT_DIR/scripts/cargo-cache.sh"

ASSETS_DIR="${MAEKON_SMOKE_ASSETS_DIR:-dist}"
INSTALL_SCRIPT="${MAEKON_INSTALL_SCRIPT:-$ROOT_DIR/scripts/install.sh}"
HOST="${MAEKON_SMOKE_HOST:-127.0.0.1}"
PORT="${MAEKON_SMOKE_PORT:-18090}"
RUN_UPDATER_TESTS="${MAEKON_SMOKE_RUN_UPDATER_TESTS:-1}"
REQUIRE_SIGNATURE="${MAEKON_SMOKE_REQUIRE_SIGNATURE:-0}"
ASSET_NAME="${MAEKON_SMOKE_ASSET_NAME:-}"
INSTALL_DIR="${MAEKON_SMOKE_INSTALL_DIR:-}"

# Path that tauri::generate_context!() validates at compile time (from tauri.conf.json).
FRONTEND_DIST="$ROOT_DIR/crates/maekon-web/frontend/dist"
...
detect_asset_name() {
  ...
  case "$os_name" in
    Darwin)
      printf 'maekon-macos-universal.tar.gz'
      ;;
    Linux)
      case "$arch_name" in
        x86_64 | amd64)
          printf 'maekon-linux-x64.tar.gz'
          ;;
        *)
...
TARGET_BIN="$INSTALL_DIR/maekon"
[[ -x "$TARGET_BIN" ]] || fatal "Installed binary not found: $TARGET_BIN"

TARGET_SIDECAR="$INSTALL_DIR/maekon-sandbox-worker"
if [[ "$(uname -s)" == "Darwin" ]]; then
  APP_BUNDLE="${MAEKON_APP_DIR:-$HOME/Applications}/Maekon.app"
  APP_SIDECAR="$APP_BUNDLE/Contents/MacOS/maekon-sandbox-worker"
...
if [[ "$RUN_UPDATER_TESTS" == "1" ]]; then
  ...
  "$CARGO_CMD" test --manifest-path "$ROOT_DIR/Cargo.toml" -p maekon-app release_reliability_ -- --nocapture
fi
