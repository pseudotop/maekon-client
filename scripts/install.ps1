[CmdletBinding()]
param(
    [string]$Version = $(if ($env:MAEKON_VERSION) { $env:MAEKON_VERSION } else { "latest" }),
...
function Test-Signature {
    ...
    $scriptPath = Join-Path $env:TEMP ("maekon-verify-signature-" + [Guid]::NewGuid().ToString("N") + ".py")
    @'
import base64
import sys
...
try {
    ...
    Write-Info "Open a new terminal and run: maekon"
} finally {
