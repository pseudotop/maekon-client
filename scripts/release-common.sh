...
FRONTEND_PACKAGE_JSON="${REPO_ROOT}/crates/maekon-web/frontend/package.json"
...
try:
    ...
    for package in tomllib.loads(content).get("package", []):
        ...
        if not isinstance(name, str) or not name.startswith("maekon-"):
            continue
    ...
    def flush() -> None:
        if not name or not name.startswith("maekon-") or has_source:
            return
        if version != expected:
...
allowed_promotion_file() {
  case "${1}" in
    Cargo.toml|Cargo.lock|CHANGELOG.md|crates/maekon-web/frontend/package.json)
      return 0
      ;;
    *)
      return 1
